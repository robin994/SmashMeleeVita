...
The physical v3.98 run no longer fails in the renderer first: it reaches Training, creates Mario and advances through Yoshi setup through `collision_camera_shadow_pass`, then deterministically stops in `mpCollInterpolateECB()` at the `HSD_ASSERTREPORT` that checks the fighter ECB for NaNs. This localizes the immediate failure to a non-finite collision envelope produced from the fighter's HSD joint transforms rather than a random vitaGL fault. The supplied core does not identify a more specific instruction than the panic already recorded by the runtime log, so v3.99 fixes the unsafe producer/consumer boundary instead of suppressing that one assertion.

The HSD matrix path now has a Vita-only finite-state barrier. `HSD_MtxSRT()` and `HSD_MtxSRTQuat()` no longer evaluate `1/0` for parent-scale compensation: zero/subnormal/non-finite parent scales collapse that compensated axis rather than producing an infinity that can become `0*Inf -> NaN` in descendants. `HSD_JObjMakeMatrix()` sanitizes a malformed non-finite scale/translation/rotation before building the world matrix and reports `VITA_ARM32_JOBJ_SRT_SANITIZE` for the first occurrences. This is intentionally upstream of collision so one bad animated scalar cannot poison every child matrix.

The fighter/ECB boundary is also validated structurally. `ft_80081B38()` now checks the six retail x44 collision-joint indices against the fighter part count, handles missing joints/x44 data, rejects non-finite collision/ledge scalars and non-finite fighter scale, and substitutes the fighter root as a fail-closed source where possible. `ft_80081C88()` applies the same finite checks when collision dimensions are rescaled. `mpColl_LoadECB_JObj()` validates every transformed source joint and the resulting desired ECB; `mpCollInterpolateECB()` accepts neither NaN nor infinity and restores the last finite ECB (or a small finite default only when no valid history exists). New diagnostics are `VITA_ARM32_ECB_SOURCE_REPAIR`, `VITA_ARM32_ECB_JOINT_INVALID`, and `VITA_ARM32_ECB_REPAIR`. The original non-Vita assertion behavior is unchanged.

The Vita compilation baseline is hardened globally with `-fno-strict-aliasing`, `-fwrapv`, `-mno-unaligned-access`, and the existing `-fno-short-enums` across portable/assets/HSD/gameplay/frontend targets. A mismatched callback cast in `mpcoll.c` was replaced by an ABI-correct typed wrapper. Two `tydisplay.c` helpers that were plain external `inline` functions are now `static inline`, fixing C11 ARM linkage that otherwise left `_tyDisplay_80318CB4_sort` and `_tyDisplay_80319994_sort` unresolved at final link. The retail disc root used by the generated SIS font is now a configurable CMake path, allowing isolated worktrees to build against the source checkout's extracted GALE01 data without copying game assets.

Validation: the complete `melee_game_boot` ARM32 target compiles, the final `melee_vita` ELF links, VELF/SELF conversion succeeds, and the VPK packages successfully. The independent object audit still has its existing broad-project baseline of 75 unsupported/decomp units, but all five changed runtime units (`ft_081B.c`, `mpcoll.c`, `tydisplay.c`, `jobj.c`, `mtx.c`) compile successfully in that audit; `git diff --check` is clean. The Vita linker still emits its longstanding EABI attribute warning that project objects use 32-bit enums while the final output also links SDK objects marked variable-size; the project-side ABI remains explicitly forced to 32-bit and the warning is deliberately not hidden. Runtime marker is `MELEE_VITA_GAME_BOOT v3.99-arm32-safe`; APP_VER is 01.09. Hardware artifact: `build/vita-arm32-safe/SmashMeleeVita-assets.vpk`, 3,313,765 bytes, SHA-256 `9f8b6dd64ea8730fed54160a0135bb0f87795bc89177365621a5772d7140ffd8`.

Next physical test: repeat the same Training/Yoshi path that ended at `mpcoll.c:701`. The old `HSD_PANIC ... mpcoll.c:701` should be gone. If malformed source data is encountered instead, capture the first `VITA_ARM32_JOBJ_SRT_SANITIZE`, `VITA_ARM32_ECB_SOURCE_REPAIR`, `VITA_ARM32_ECB_JOINT_INVALID`, or `VITA_ARM32_ECB_REPAIR` line; that marker identifies the upstream class and offending joint without allowing the invalid float to propagate into a later unrelated crash. This build is a safety baseline, not a claim that every remaining GameCube-to-ARM assumption in the whole decomp has already been eliminated.
...
The physical v3.96 Training/Castle run is the first build to show gameplay for an instant. The log confirms that the new Castle GX path is genuinely active: frame 1 has 19,509 finite projected vertices, 10,734 inside XY/XYZ, hsd_multitex=35, gl_error=0 and capture_result=0. Frames remain capture-clean until DamageLw3 begins; only then does the independent immediate-particle NaN path start reporting capture_result=-2. Stage/fighter display lists continue to be captured after that error, so the particle failure is not treated as the primary reason the already-visible world becomes black.

Source tracing found a direct data boundary at the same gameplay event. ftCo_Damage triggers lbBgFlash_80021C48(), which drives the full-screen overlay from LbBf.dat. That archive was routed through mv_gameplay_archive_prepare_raw(), but the dispatcher only recognized fighter ftData, PlCo and ItCo roots; public symbol lbBgFlashColAnimData was therefore left in GameCube packed-bitfield byte order. On little-endian ARM, the first retail command word 0x48000000 (ColorOverlay opcode 18) aliases the wrong opcode. v3.97 adds a schema-specific LbBf converter: it preserves the 16 relocation-owned script pointers and byte-sized priorities, while converting only the reachable packed ColorOverlay command words with the same proven command converter used by PlCo.

A linked-ARM regression uses the real orig/GALE01/files/LbBf.dat and executes the exact damage-flash IDs 2-6 through lb_013B.c after nativeization. All five scripts now decode as op18 -> op19(12) -> op11(12) -> op12 -> op10, keep finite interpolated colors, fade alpha from 12 to 0, terminate, and preserve relocation/color payload bytes. The test is available as lbbf-color-check and is included in endian-preflight. Existing Castle multitexture/TEV, Training raw, full stage corpus including GrCs.dat, camera quake, ItCo Food/HSD, GX NBT, projection/culling, no-GX-MMIO, HPS playhead, shared mini-DAT relocation and fighter-command regressions all pass. Runtime marker is MELEE_VITA_GAME_BOOT v3.97 and APP_VER is 01.07. Hardware artifact: build/vita-full/SmashMeleeVita-v3.97-lbbf-damage-flash-native.vpk, 3,309,737 bytes, SHA-256 7ed0d39b484b3f9df1590b334446c2ad9bc905e2032bb8ab6809ba3a64e6a1a2.

Next physical test: repeat the same Training/Castle route. The expected change is that the brief visible gameplay no longer gets replaced by a malformed persistent black damage overlay when DamageLw3 fires. The known particle immediate path may still report first_error_line=944 / capture_result=-2 after damage; treat that as a separate effect-particle producer bug unless the world itself still disappears.

# Porting status - 2026-09-14, v3.96 exact Castle GX multitexture/TEV replay

## 2026-09-14 — v3.96: replace the black-screen gameplay raster approximation with the exact retail Castle GX graph

The physical v3.95 Training/Castle run closes the previous camera/matrix investigation: gameplay remains black, but the captured frame is now healthy upstream of rasterization. The frame contains thousands of finite vertices with thousands inside the full frustum, capture_errors=0, and vitaGL reports no GL error across sustained gameplay. Projection, view/PNMTX state, GX packet capture and the known-good clockwise culling baseline are therefore left unchanged. The first draw with geometry inside the frustum carries attr_mask=0x00006a00, i.e. POS + CLR0 + TEX0 + TEX1, and has PE state that permits color output. The remaining defect is in the GX material/vertex translation into vitaGL.

The renderer audit found four concrete representation errors rather than another visibility guess. MvGxCaptureVertex stored only TEX0 even when the retail display list supplied TEX1, and vitaGL consequently fed v->tex0 to both texture units. Vertex presence used compact internal bits while the replay tested 1 << GX_VA_*; CLR0 was therefore decoded but never recognized by the renderer. HSD post-texture matrices were not tied to the exact GX matrix IDs used by each texgen, which loses Castle atlas transforms. Finally, the texture-cache baker could pre-apply material_rgba to TEX0 even though the actual gameplay TEV graph starts from raster/vertex color. v3.96 changes presence tracking to real GX attribute bits, stores TEX1 independently, records exact texgen sources and post-texture-matrix IDs, and keeps raw texture samples for the exact HSD multitexture graph.

The fix is grounded in the retail asset and HSD compiler rather than inferred from screenshots. A linked-ARM regression loads and nativeizes orig/GALE01/files/GrCs.dat, constructs the actual HSD runtime graph, finds exactly nine two-TObj materials, and confirms every one routes the first TObj from GX_TG_TEX0 and the second from GX_TG_TEX1. Running HSD_MObjSetup on a real Castle material captures the generated two-stage GX program: stage 0 uses coord0/map0 and computes RGB = RASC * TEX0; stage 1 uses coord1/map1 and computes RGB = PREVIOUS * TEX1; the final alpha is preserved from RASA. The same run confirms GX_PTTEXMTX0 and GX_PTTEXMTX1 are both present, with the second layer carrying a real non-identity atlas transform. mv_gx_material_multitex_hsd_modulate() accepts only this exact graph.

vitaGL now reproduces that graph directly with two fixed-function texture units. Unit 0 combines PRIMARY_COLOR * TEX0 for RGB while preserving primary alpha; unit 1 combines PREVIOUS * TEX1 for RGB and preserves prior alpha. Texture coordinates are selected from the captured texgen source, so TEX1 receives v->tex1 rather than a duplicate of TEX0, and each unit applies its own captured affine UV matrix. The exact path additionally requires CLR0, TEX0 and TEX1 in the command attr mask before claiming support. A new source regression checks this full bridge, while the ARM Castle regression verifies the real HSD program and matrix routing. VITAGL_REPLAY_CAPTURED_SUBMIT now includes hsd_multitex=N as a non-invasive hardware confirmation that the exact path was exercised.

Final validation on the v3.96 ELF/VPK: stage-multitex-check PASS; vitagl-multitex-check PASS; Training raw nativeization PASS; complete stage-raw corpus PASS including GrCs.dat; SSS ShapeAnim PASS; GX NBT3 and linked-ARM NBT PASS; GX projection/depth and CW culling baseline PASS; no direct GX FIFO MMIO PASS; SIS font PASS; HPS playhead endian PASS; shared mini-DAT relocation PASS; fighter-command endian PASS; git diff --check clean. Runtime marker is MELEE_VITA_GAME_BOOT v3.96 and APP_VER is 01.06. Hardware artifact: build/vita-full/SmashMeleeVita-v3.96-castle-multitex-tev.vpk, 3,309,334 bytes, SHA-256 4f86765ba5f698e4226fa9eb53024448ca2153c5e3740fffbc36d38e973ad2f2. No commit or push was performed.

Next physical test: repeat the same Training/Castle route. The decisive line is VITAGL_REPLAY_CAPTURED_SUBMIT; hsd_multitex must be greater than zero on a frame containing the Castle dual-texture material, with gl_error=0. The expected visual change is that the first in-frustum Castle draw now receives the real CLR0, distinct TEX0/TEX1 coordinates, both atlas matrices and the exact two-stage TEV operation. If the screen remains black despite hsd_multitex>0 and gl_error=0, do not revisit camera/projection/PNMTX: the next boundary is framebuffer/present or an unsupported vitaGL fixed-function combine behavior.

# Porting status - 2026-09-14, v3.95 ARM-safe quake camera layout + Food special-attribute nativeization

## 2026-09-14 — v3.95: remove the GameCube linker-layout dependency and close the Food Data Abort

The physical v3.94 Training run is a real rendering advance rather than another all-black matrix failure. Frame 1 now captures 19,509/19,509 finite projected vertices, 6,813 vertices inside the full XYZ frustum and capture_errors=0/capture_result=0; frame 120 is also capture-clean. The previous all-NaN view/PNMTX failure is therefore gone. The same run still reports VITA_CAMERA_TRANSFORM_INVALID solely because game_camera.translation is NaN while the gameplay transform itself is finite, and the user observed one brief distorted/incorrectly colored frame before returning to effectively black output.

Source and linked-ELF audit identifies the exact producer. Camera_ApplyQuake() reconstructed a synthetic CameraStaticData aggregate by casting &cm_803BCB18 and assuming the callback table, two WObj descriptors and cm_803BCB64 were contiguous as in the original GameCube link layout. The Vita ELF disproves that assumption: cm_803BCB18 and cm_803BCB64 are separated by roughly 0x31900 bytes and live in different placement regions. The quake code consequently read unrelated bytes as aspect/viewport and generated NaN translation even though the actual GrCs quake animation is valid. v3.95 removes the fake aggregate and reads cm_803BCB64.aspect/viewport directly. A permanent camera-quake-check verifies the source path and linked symbol separation. The stage-quake ARM regression was also upgraded from descriptor-only validation to executing all four real GrCs quake animations through HSD_JObjLoadJoint/AddAnim/ReqAnim/AnimAll: all 124 sampled translations are finite. The first hardware calculation now logs VITA_CAMERA_QUAKE_LAYOUT_PASS; VITA_CAMERA_TRANSFORM_INVALID should no longer occur for the normal Castle quake path. Projection/depth conversion and CW culling are unchanged.

The supplied v3.94 core dump is a second independent issue, also resolved structurally. It expands to an ARM ELF core whose main thread stops on a Data Abort. With the runtime RX relocation applied, PC resolves to HSD_JObjLoadJoint+0x6 and LR to Item_802680CC. Stack reconstruction gives Item_8026862C -> itFoods_Logic18_Spawned -> it_80273318 -> Item_802680CC -> HSD_JObjLoadJoint. The crashing Item is kind 0x12 (It_Kind_Foods), and xC8_joint contains the invalid pointer 0xAC9E831B. Retail ItCo.dat proves why: Food specialAttributes starts with count=28, but Article::x4_specialAttributes was not converted by the generic ItCo pass, so ARM read the raw big-endian count as 0x1C000000 (469,762,048) and HSD_Randi indexed far outside the 28-food table. v3.95 adds a schema-specific Food converter rather than swapping generic item attributes: it nativeizes count, heal values and placement floats, preserves relocation-owned HSD_Joint pointer words, and adds all 28 Food model roots to the normal typed HSD graph conversion. The linked ARM regression now covers 94 Articles / 92 generic models / 28 Food variants / 106 unique HSD roots and successfully relocates, loads and removes all 106 roots.

Final validation on the v3.95 ELF/VPK: camera-quake-check PASS; ItCo item HSD+Food PASS (food=28, joint_roots=106, loaded=106); stage-quake endian/runtime PASS (124 finite samples); GX projection/culling PASS; GX NBT ARM PASS; no direct GX FIFO MMIO PASS; HPS playhead endian PASS; shared mini-DAT relocation PASS; fighter command endian PASS; git diff --check clean. Runtime marker is MELEE_VITA_GAME_BOOT v3.95 and APP_VER is 01.05. Hardware artifact: build/vita-full/SmashMeleeVita-v3.95-camera-layout-food-native.vpk, 3,307,371 bytes, SHA-256 35f264e682060a8387d79f7dc051cd8e453aa37dd9a7dcf432225cf8d869ecf4. No commit or push was performed.

Next physical test: repeat the same Training/Castle route. Expected positive markers are VITA_STAGE_QUAKE_HSD_RAW_NATIVE_PASS, VITA_CAMERA_QUAKE_LAYOUT_PASS with finite translation, and VITA_ITCO_FOOD_NATIVE_PASS count=28. The prior VITA_CAMERA_TRANSFORM_INVALID translation=nan,nan and Food HSD_JObjLoadJoint Data Abort should be absent. If geometry is then visible but colors/materials remain wrong, the next investigation moves to GX/TEV material replay using the already-finite camera/PNMTX baseline; do not alter projection or culling speculatively.

# Porting status - 2026-09-14, v3.94 stage quake HSD endian fix

## 2026-09-14 — v3.94: fix the source of the gameplay camera NaN on Princess Peach’s Castle

The physical v3.93 Training run remained black, but its new HSD matrix probe closed the render-side ambiguity: VITA_JOBJ_PMTX_INVALID reports view_finite=0 and world_finite=1. The JObj world matrix is a valid 0.5-scale matrix while the CObj view matrix is already entirely NaN before HSD_JObjMakePositionMtx multiplies view*world. No VITA_CAMERA_TRANSFORM_INVALID marker appears, so the dynamic transform itself passed the v3.93 finite check. Projection remains finite. This excludes the stage/fighter world JObj, GX projection/culling and vitaGL replay as the producer of the global NaN PNMTX.

Source tracing found a complete upstream path specific to the selected Training stage (St_Kind_Castle): grCastle_801CE860 continuously requests QuakeKind_Loop; grLib_801C9CEC loads the independent stage public quake_model_set, attaches anims[0], and grLib_801C9BC8 copies the animated JObj translation into Camera_SetQuakeOffset. Camera_ApplyQuake then derives game_camera.translation, which Camera_8002AF68 adds to both CObj eye and interest immediately before HSD_CObjSetupViewingMtx/C_MTXLookAt. The v3.93 guard checked transform position/interest/FOV but not this post-transform quake translation.

The structural bug was in vita/stage_archive_vita.c: mv_stage_archive_prepare_raw nativeized map_head JObjs and their animation/camera/light/fog graphs, map_plit and dynamics, but never visited the separate quake_model_set DynamicModelDesc. The retail GrCs.dat proves the gap: quake_model_set is at 0x16c5e8, points to joint 0x16c318 and four AnimJoint roots, and its first FObjDesc has big-endian length=5. If consumed directly by little-endian ARM, that field becomes 0x05000000 (83,886,080). HSD_FObjLoadDesc copies desc->length and the other serialized scalar fields without endian conversion, so the unconverted quake animation can corrupt its translated JObj and poison only the camera/view while ordinary world JObjs remain valid.

v3.94 nativeizes the complete quake_model_set graph at the existing pre-relocation HSD boundary using the same typed converters as normal stage animation: joint, all four AnimJoint/AObj/FObj chains, optional MatAnim and ShapeAnim tables. It shares StageHsdRawContext::seen with the map graph, preventing double conversion. Pointer relocation fields remain untouched for HSD_ArchiveParse. Camera_8002AF68 is also hardened so game_camera.translation must be finite before it is applied; this is a fail-closed secondary guard, not the root fix. Projection, CW culling, NBT handling, 256 MiB heap and ATTRIBUTE2=12 are unchanged.

A new linked-ARM regression, vita/tools/test_stage_quake_arm.py / stage-quake-check, runs against the real orig/GALE01/files/GrCs.dat. It verifies all 4 quake AnimJoint roots and 8 FObj descriptors become host-native, that the first length remains 5 rather than 0x05000000, and that DAT pointer bytes are preserved. The test passes. The complete stage-raw corpus also passes every stage including GrCs.dat; stage-map-plit-check passes 72 files / 191 lights; stage-light-check passes 508 sets / 1101 lights. gx-nbt-arm-check, gx-projection-check, gx-fifo-check, hps-playhead-check, lbarchive-relocate-check and fighter-command-check also pass.

Runtime marker: MELEE_VITA_GAME_BOOT v3.94. APP_VER: 01.04. Hardware artifact: build/vita-full/SmashMeleeVita-v3.94-stage-quake-native.vpk, 3,306,295 bytes, SHA-256 ab0a024a795a22d6f7194eda60d27045e9edd86c6dc87bc3d379db7e80eebb44.

Next physical test: repeat the same Training route on Princess Peach’s Castle. The primary success criterion is that VITA_JOBJ_PMTX_INVALID no longer reports view_finite=0 and command 0 no longer carries an all-NaN position matrix. If the former NaN view was the only visibility blocker, stage/fighter geometry should become visible. The independent 13-vs-12 immediate packet mismatch may still leave capture_result=-2 and will be addressed separately once camera-space geometry is finite.

# Porting status - 2026-09-14, v3.93 gameplay camera finite-state guard

## 2026-09-14 — v3.93: stop a non-finite gameplay camera from poisoning every world PNMTX

The physical v3.92 Training run remains black but resolves the previous ambiguity: command 0 has pos_loaded=1 and its captured 3x4 PNMTX is entirely NaN, while the 4x4 GX projection is finite and numerically sane. Frame 1 still captures 859 commands / 19,506 vertices / 13,190 triangles and reports 276 position-matrix loads, so this is not an unbound GX matrix slot. The invalidity exists before vitaGL rasterization.

Source audit ruled out the established GX projection/depth conversion, C_MTXLookAt implementation, PSMTXConcat implementation, CObj/WObj dirty initialization, HSD_JObjMakePositionMtx concat order, PObj matrix-mark lifecycle, the Castle grGroundParam schema, fighter camera x3C vectors, and camera target-bone index/offset data. GrCs.dat has sane camera values (zoom 120, max depth 1000, pan -10, track ratio/smooth 1), Mario camera vectors are 10,22,-9 / 16,-9,10.8 and bone 5 offset 0,2,0; Samus uses 14,22,-9 / 16,-9,14.5 and bone 19 offset 0,0,0.

v3.93 hardens the correct state boundary: Camera_8002AF68 now rejects a non-finite or degenerate dynamic camera transform on Vita instead of overwriting an already-valid HSD_CObj eye/interest/FOV. The existing CObj state originates from the valid static gameplay camera descriptor, so rejection preserves the last valid camera rather than injecting an identity or renderer-side fake. The first rejection logs VITA_CAMERA_TRANSFORM_INVALID plus every CmSubject position/extent/bone value to identify the producer. HSD_JObjMakePositionMtx also logs the first invalid product as VITA_JOBJ_PMTX_INVALID together with independent view_finite/world_finite classification and both matrices, so a remaining failure distinguishes camera/view corruption from JObj/world corruption in one run. Projection, CW culling, NBT handling, 256 MiB heap and ATTRIBUTE2=12 are unchanged.

Validation: full ARM compile/link/VELF/SELF/VPK passes; gx-nbt-arm-check, gx-projection-check, gx-fifo-check, hps-playhead-check, lbarchive-relocate-check and fighter-command-check pass; git diff --check is clean. Runtime marker is MELEE_VITA_GAME_BOOT v3.93, APP_VER 01.03. Hardware artifact: build/vita-full/SmashMeleeVita-v3.93-camera-finite-guard.vpk, 3,305,817 bytes, SHA-256 ebd2c9c2d00da92d759ba4a1ae405778c2145bd1b0c305d5135867f3aa8b8ba2.

Next physical test: repeat Training. If VITA_CAMERA_TRANSFORM_INVALID appears, the camera guard should keep the previous valid CObj and the world PNMTX/clip probe should become finite, potentially making stage geometry visible immediately; inspect the accompanying VITA_CAMERA_SUBJECT lines to identify the upstream bad subject. If no camera-invalid marker appears but VITA_JOBJ_PMTX_INVALID reports view_finite=1/world_finite=0, the next fix is in the corresponding JObj/skeleton transform path.

# Porting status - 2026-09-14, v3.92 first invalid model-view/clip matrix probe

## 2026-09-14 — v3.92: localize the black frame to model/view matrix state rather than projection/culling guesswork

The physical v3.91 Training run is still fully black, but its clip-space probe materially narrows the failure. Frame 1 contains 858 captured commands, 19,505 vertices and 13,191 triangles; vitaGL submits 658 commands with gl_error=0. However only 447/19,505 tested vertices have finite non-zero-W clip coordinates, only 411 have positive W, and only 196 are inside the full XYZ frustum. The first finite command is 776 and the first inside command is 789. Therefore more than 97% of the gameplay geometry is already invalid before rasterization; downstream culling, texture and PE state cannot by themselves explain the global black frame.

The first inside command (789) uses TEX_COORD_HILIGHT and is conservatively marked MV_GX_MATERIAL_UNSUPPORTED_TEXCOORD. That is a real missing replay feature, but it is not treated as the primary black-screen cause because the bulk world geometry preceding it has invalid/zero clip W. Audits against the local Dolphin implementations show C_MTXLookAt and PSMTXConcat are semantically identical, the Training camera descriptor is sane (eye around 0,40,300; near 0.1; far 16384), WObj initialization marks the camera dirty, HSD_JObjMakePositionMtx performs view*world, and HSD_JObjDispSub clears PObj matrix marks per JObj. No projection/culling/material workaround is introduced.

v3.92 extends the first-frame probe to record the first non-projectable command/vertex, full 3x4 position matrix, full 4x4 GX projection, source vertex, view-space xyz, clip xyzw, finite/zero tests, and whether every referenced PNMTX slot had actually been loaded. MvGxCaptureCommand now carries pos_mtx_valid; GXLoadPosMtxImm marks loaded slots, and PNMTXIDX draws aggregate validity across the per-vertex matrix palette. VITA_RETAIL_PRESENT also reports pos_mtx_loads and nrm_mtx_loads. This cleanly distinguishes an uninitialized capture slot from a matrix produced incorrectly upstream.

Validation: full ARM compile/link/VELF/SELF/VPK passes; linked-ARM gx-nbt-arm-check, gx-projection-check and gx-fifo-check pass after the final matrix-validity change, while the earlier full v3.92 regression run also passed gx-nbt3, HPS playhead, lbArchive relocation and fighter-command endian checks. git diff --check is clean. Runtime marker is MELEE_VITA_GAME_BOOT v3.92 and APP_VER is 01.02. Hardware artifact: build/vita-full/SmashMeleeVita-v3.92-first-bad-matrix-probe.vpk, 3,305,026 bytes, SHA-256 873a7afd2e432f7b6d2a6213dfcdf27908c78cee0f8f6fe475ed20ad1b681049.

Next physical test: repeat the same Training route and return runtime.log. The decisive lines are VITA_RETAIL_FIRST_BAD_CLIP, VITA_RETAIL_FIRST_BAD_POSMTX, VITA_RETAIL_FIRST_BAD_PROJ and the first VITA_RETAIL_PRESENT. If pos_loaded=0, fix GX matrix-state lifecycle. If pos_loaded=1 and pos_zero/non-finite, trace the producing HSD JObj/PObj path. If both position and projection matrices are finite/non-zero but W is still zero, inspect the exact transformed Z convention for that command.

# Porting status - 2026-09-13, v3.91 GX packet-size + clip-space diagnostics

## 2026-09-13 — v3.91: distinguish the 13/12 immediate packet mismatch from the actual black-screen visibility failure

The physical v3.90 Training run remains black but is stable through at least frame 75 and contains no HSD panic, no attack-power assertion, and no VITA_THROW_DAMAGE_* guard activation. Gameplay continues to load real Mario/Donkey FigaTrees while vitaGL receives about 858-872 captured commands, ~19.5k vertices and ~13.2k triangles per frame. The previous input-triggered throw crash therefore does not recur in this run.

The refined v3.90 error tuple is first_error_line=861 / error_args=255,4294967295,3085. 3085 is 0x00000C0D: decode_vertex() returned success and consumed 12 bytes while the immediate-mode packet calculator expected 13. The unset reason/attribute fields prove this is not an unsupported component decoder; it is a one-byte disagreement between active GX VCD state and the bytes actually emitted by the immediate writer. v3.91 logs the first mismatch as VITA_GX_PACKET_MISMATCH with command index, primitive, vtxfmt, vertex count, active-attribute mask, packed GXAttrType state, current matrix and NBT mode.

Because the mismatch occurs only after roughly 859 commands have already been captured, v3.91 also probes the already-valid geometry before presentation instead of assuming the packet error alone explains the fully black frame. VITA_RETAIL_CLIP_PROBE computes model->GX projection->OpenGL depth-converted clip/NDC statistics for all projected triangle vertices and reports positive-W, inside-XY and inside-XYZ counts plus NDC ranges. VITA_RETAIL_DRAW_STATE_PROBE records the first command with geometry inside the XY frustum, including viewport, culling, texture/material, PE blend, depth test/write, color/alpha updates and alpha compare state. No rendering state is changed by these diagnostics; the established GX depth conversion, CW culling, NBT alias semantics, 256 MiB heap and ATTRIBUTE2=12 remain unchanged.

Validation: full ARM compile/link/VELF/SELF/VPK passes; gx-nbt3-check, linked-ARM gx-nbt-arm-check, gx-projection-check, gx-fifo-check, hps-playhead-check, lbarchive-relocate-check and fighter-command-check pass; git diff --check is clean. Runtime marker is MELEE_VITA_GAME_BOOT v3.91 and APP_VER is 01.01. Hardware artifact: build/vita-full/SmashMeleeVita-v3.91-gx-packet-clip-probe.vpk, 3,303,557 bytes, SHA-256 2c093ee2bdb834525ffc22dedb4305ac1ea263e1babc026007fc2648d5d9f3b6.

Next physical test: enter the same Training route and preserve runtime.log. The first VITA_GX_PACKET_MISMATCH line identifies the exact stale/extra descriptor behind the 13/12 packet. The two clip/draw-state lines determine in the same run whether the black screen is caused by geometry outside clip space or by downstream PE/material/depth state.


## 2026-09-13 — v3.90: turn the remaining black-screen capture failure into an attributed GX error and keep input-triggered throw corruption observable

The physical v3.89 run reaches Training and continuously presents through vitaGL, but remains black. The new telemetry makes the renderer failure deterministic: frame 1 reports 858 commands, 19,505 vertices and 13,191 triangles with capture_result=-2 and first_error_line=843 / error_args=0xffffffff,12,13. That line is the immediate-mode decode_vertex() failure: the decoder has consumed 12 of a computed 13-byte vertex and returns -1. The v3.89 NBT alias work therefore materially increased captured gameplay geometry relative to the earlier ~16.5k-vertex run, but one vertex attribute/state mismatch still aborts the capture. Projection, GX depth conversion and the known-good CW culling mapping are deliberately unchanged.

v3.90 replaces the generic decode failure with structured telemetry. decode_vertex() now records the first failing attribute, GXAttrType and reason (invalid element size, direct underrun, indexed underrun, missing indexed array/stride, or unsupported type). The VITA_RETAIL_PRESENT error_args now encode reason, GX attribute, and a packed attr-type/vtxfmt/cursor/vertex-size word. The next physical black frame can therefore identify the exact attribute responsible for the 12/13 mismatch instead of another renderer experiment.

The same v3.89 run later panics at ftcoll.c after random controller input with attack power 234881024.0. This is separate from the black renderer. The panic is reached from the throw/capture damage path, whose retail Mario/Captain throw scripts contain only small damage values. v3.90 adds a Vita-only guard at the throw-hitbox construction boundary: a >500 integer is byte-swapped only when that produces a sane <=500 retail value, with VITA_THROW_DAMAGE_ENDIAN_FIX telemetry. If the subsequent smash scaling alone produces an impossible magnitude, VITA_THROW_DAMAGE_SCALE_INVALID records base damage plus smash state/frames/hold/multiplier and falls back to the unscaled throw damage so the physical test can continue. No general collision or normal-hit damage clamp was added.

Validation on the authoritative checkout: full ARM compile/link/VELF/SELF/VPK succeeds; gx-nbt3-check, gx-nbt-arm-check, gx-projection-check, gx-fifo-check, hps-playhead-check, lbarchive-relocate-check and fighter-command-check pass; git diff --check is clean. Runtime marker is MELEE_VITA_GAME_BOOT v3.90, APP_VER 01.00, heap remains 256 MiB and ATTRIBUTE2=12. Hardware artifact: build/vita-full/SmashMeleeVita-v3.90-gx-decode-reason-throw-guard.vpk, 3,301,734 bytes, SHA-256 ba84c1151996275f33956e98be7db59170854b4cc359f6f1b9c7ea295fc3010f.

Next physical test: repeat Training and let at least the first black frame present. Return runtime.log. For v3.90 error_args=R,A,P means R=decode reason, A=GXAttr enum, and P packs attr_type in bits 31..24, vtxfmt in 23..16, consumed bytes in 15..8 and computed vertex bytes in 7..0. Also exercise the same inputs that caused the v3.89 throw panic; any endian or smash-scaling corruption should now be logged instead of terminating the run.

# Porting status - 2026-09-13, v3.89 GX NBT alias + shared FigaTree relocation

## 2026-09-13 — v3.89: align GX NBT hardware semantics and fix Popo→Nana shared mini-DAT rebasing

The v3.88 physical run did not reach the new GX first-error telemetry because match construction stopped earlier while creating Nana/Ice Climbers. The final log marker is a deterministic `lbArchiveRelocate: byte-order mismatch` followed by the retained `ftdata.c` panic in the branch that copies a previously loaded shared fighter mini-DAT. On Vita the source mini-DAT header/relocation metadata remains serialized big-endian while its relocation-backed data words already contain live ARM pointers; the original `lbArchiveRelocate()` interpreted the header and relocation table as native-endian. v3.89 gives this function the same big-endian metadata treatment already used by `HSD_ArchiveParse()` and then rebases the already-relocated data words by the copy delta. `test_lbarchive_relocate_arm.py` reproduces exactly parse → copy → relocate and passes.

Cross-checking the GX capture layer against retail Dolphin semantics and the independent `melee-unlocked` GX runtime exposed a second systemic mismatch. `GX_VA_NBT` is not a separate FIFO attribute: it aliases the hardware normal VCD/VAT/array slot used by `GX_VA_NRM`, while enabling three-vector normal/binormal/tangent payload semantics. The Vita shim previously stored `GX_VA_NBT` in enum slot 25 even though the vertex decoder consumes the real hardware attribute range only through `GX_VA_TEX7`; therefore some HSD NBT descriptors could be omitted from the computed vertex packet even after the earlier NBT3 decoder work. v3.89 canonicalizes `GXSetArray`, `GXSetVtxDesc` and `GXSetVtxAttrFmt` NBT state onto the NRM slot while retaining an explicit `normal_is_nbt` flag. Direct HSD NBT with `GX_NRM_XYZ` still consumes nine scalars; indexed `GX_NRM_NBT3` still consumes three independent indices. The linked-ARM `test_gx_nbt_arm.py` feeds a synthetic triangle through the compiled GX capture API and verifies all nine N/B/T floats plus zero capture errors.

The `melee-unlocked` comparison also confirms that our position-matrix index treatment is semantically equivalent: PNMTXIDX values are raw XF row indices (0,3,6,...) and three consecutive rows form one position matrix. The Vita capture stores those matrices by `id/3` and flattens indexed vertices before replay, which is equivalent for visibility. Its projection sign corrections were not copied blindly: that runtime operates on raw signed XF viewport half-width/half-height registers, whereas the Vita shim captures the API-level positive `x,y,w,h`; our OpenGL bottom-left viewport conversion already accounts for the normal GX Y inversion. The established `z_gl = 2*z_gx + w` depth conversion and known-good culling mapping therefore remain unchanged.

Validation: full ARM compile/link/VELF/SELF/VPK passes; `gx-nbt3-check`, linked-ARM `gx-nbt-arm-check`, `gx-projection-check`, `gx-fifo-check`, `hps-playhead-check` and linked-ARM `lbarchive-relocate-check` pass; `git diff --check` is clean. Runtime marker is `MELEE_VITA_GAME_BOOT v3.89`, APP_VER `00.99`, heap remains 256 MiB and SFO `ATTRIBUTE2=12`. Hardware artifact: `build/vita-full/SmashMeleeVita-v3.89-gx-nbt-alias-shared-relocate.vpk`, 3,301,598 bytes, SHA-256 `84624fea1414a8a5b5a3694632575f0ab774e583b9097eeae8917fa8ca3d1a37`.

Next physical test: repeat the same Training route. The v3.88 Nana `lbArchiveRelocate` panic must be gone. If the match still presents black, the retained v3.88 telemetry now prints `first_error_line` and `error_args` on every non-zero capture result; that value becomes the next exact GX boundary instead of another renderer guess.

## 2026-09-13 — v3.84: nativeize every colored fighter costume HSD root before relocation

The authoritative physical input is the supplied v3.83 `runtime.log`, preserved as `build/vita-full/runtime-v3.83-2026-09-13-samus-costume-ptcl.log` (31,708 bytes, SHA-256 `2d11f83b1a3ea9601d0f41e0f715855696537880c4ff39f87759e6363e245832`). The matching `psp2core-1789316475-0x000dfa38b5-eboot.bin.psp2dmp` is 1,123,840 bytes, SHA-256 `83a23fdb2088bb7723431d7392c3e2d7f4f3adb5e7747a9249c1df1e15947fb2`, and expands to an ARM EABI5 ELF core. The runtime frontier is deterministic enough that no guessed core-stack interpretation is needed for this fix.

v3.83 materially advances the real Training match path. Stage and item nativeization complete, Mario reaches `VITA_FIGHTER_CREATE_PASS`, and the opponent Samus reaches base-data, effect and costume-archive preload. The stop is immediately after `VITA_FIGHTER_CREATE_STAGE kind=13 stage=costume_archive_pass costume=2`, at the retained `jobj.c:637` guard `load-only JObj particle tree unsupported`. Unlike Mario, there is no `VITA_FIGHTER_HSD_RAW_NATIVE_PASS` marker for the Samus costume archive before the guard.

This is not a genuine Samus particle tree. PAL costume 2 is `PlSsBk.dat`, whose public model root is `PlySamus5KBk_Share_joint`. `mv_fighter_archive_prepare_raw()` only recognized names ending in the exact default-costume spelling `5K_Share_joint`, so every alternate costume with a colour token between `5K` and `_Share_joint` bypassed scalar nativeization. On little-endian ARM, ordinary big-endian JObj flag words can then alias `JOBJ_PTCL`, exactly reproducing the panic. A PAL census finds 28 default costume roots and 97 alternate-colour roots; the bug therefore affected 97 costume archives across the roster, not a Samus special case.

v3.84 fixes the schema boundary rather than weakening HSD. Fighter costume detection now accepts `Ply...5K..._Share_joint` and the corresponding optional `_Share_matanim_joint`, covering default and colour-token variants while still requiring the `Ply`/`5K` costume schema. Genuine `JOBJ_PTCL` remains fail-closed in `HSD_JObjLoadJoint`; genuine `JOBJ_INSTANCE` handling is unchanged.

A new linked-ARM regression, `vita/tools/test_fighter_costumes_arm.py`, discovers every retail PAL fighter costume archive, walks every reachable serialized JObj, runs the production `mv_boot_archive_prepare_raw()` boundary, verifies each flag word is native-endian, relocates with `HSD_ArchiveParse()`, then loads/removes the public model with the original `HSD_JObjLoadJoint()`/`HSD_JObjRemoveAll()`. Result: `PASS fighter costume HSD: archives=125 default=28 alternates=97 joints=8833 false_ptcl_fixed=116 loaded=125`; the census contains zero genuine PTCL costume nodes. `fighter-costume-check` is now part of `endian-preflight`. The existing GmTrain raw regression is also wired into the preflight as `training-raw-check`.

Cross-regressions remain green on the final v3.84 linked ELF: fighter x48 covers 23 cases / 83 Articles / 75 roots including Samus' genuine INSTANCE accessory; GmPause still corrects its two false PTCL/INSTANCE sites; both GmTrain archives still correct 20 false PTCL sites total; and `test_boot_arm.py` reaches original GM_BOOT/GS_MEMCARD after HSD/GX/audio/DVD/ARQ and all GameMode `on_init` callbacks. Full ARM compile/link, VELF, SELF and VPK packaging pass. Existing enum-size linker warnings remain non-fatal and unchanged.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.84`; VPK APP_VER is `00.94`. Final hardware artifact: `build/vita-full/SmashMeleeVita-v3.84-fighter-costume-endian.vpk`, 3,291,914 bytes, SHA-256 `25f7981c0eb7942b8b255edd13af4fa6d7e199076db3466f3c2155b804a295b7`; it is byte-for-byte identical to the generated `SmashMeleeVita-assets.vpk`. Packaged `eboot.bin` is 2,861,183 bytes, SHA-256 `abd6c4116a79ef4d45905c136c08be1f2ab2331b5de8c57c68553998bf354268`; unstripped `melee_vita` is 9,480,688 bytes, SHA-256 `abb5eafabe8856cf047a0aa8d922257ed98b5290a8bd17a5ef5f877f4f32bd14`; SFO is 912 bytes, SHA-256 `74a5f5fedca7e2a2f291275a3691be968c9338103b901d1ddb8b5f7af4d8ecbf` and contains `00.94`.

Next physical test: install exactly the named v3.84 VPK and repeat the same Training route that produced the v3.83 core. After Samus `stage=effect_pass`, the log must now contain `VITA_FIGHTER_HSD_RAW_NATIVE_PASS ... root=PlySamus5KBk_Share_joint`, then progress beyond `stage=costume_archive_pass` to `stage=costume_jobj_pass`. The old `jobj.c:637` false-PTCL panic must not recur. If execution stops later, preserve the new `runtime.log` and core: that next frontier will be beyond the now exhaustively audited costume endian boundary.

## 2026-09-13 — v3.67: replace the partial scene frontier with the original GameMode loop and remove the proven Stage Select crash

The authoritative physical input is the supplied v3.66 `build/vita-full/runtime.log`. A byte-for-byte snapshot is preserved as `build/vita-full/runtime-v3.66-2026-09-13-sss-shape-crash.log` (14,288 bytes, SHA-256 `c54f0fb697b2ee037d8e653b3a8f45c1136136adcd52e0a3a48b327fc90abbcb`). The supplied `psp2core-1789283572-0x000b9532f1-eboot.bin.psp2dmp` is 617,568 bytes, SHA-256 `cb006f40712c79ac906d4ea3ac4c5a37a0f4177c5cd16d4e615f27825686d64d`, and expands to an ARM EABI5 ELF core. Its Vita-specific notes do not expose registers to stock GDB, so the diagnosis is based on the exact final runtime marker and source assertion rather than a guessed GPU fault.

The v3.66 hardware run establishes four facts. Opening/title/main menu and the Training Character Select execute original Melee callbacks and reach vitaGL capture. SIS is not absent: the main-menu capture reports ten textured SIS commands with a valid 32x32 atlas texture, alpha, viewport and projection, while CSS reports two SIS text objects and 265 total commands. Mode 28 is `GM_TRAINING`; the old `GAME_1P_ROUTE_BEGIN mode=ADVENTURE(28)` label was wrong telemetry, not an Adventure dispatch. The deterministic stop occurs on entry to Stage Select: set 10 in `MnSlMap.usd` contains a valid PObj with flags `0x9000`, including `POBJ_SHAPEANIM`, and the compact builder rejected it before `mnStageSel` could display a frame. `mnstagesel.c:461` then deliberately panicked. This is the cause of the supplied crash.

The investigation also confirmed a deeper architectural error behind the missing loading screen and inability to enter a match. The routed VS/Training path manually called the CSS and SSS callbacks, then called only the match state's `on_enter` callback and returned to the outer Vita loop. It never executed the original `runGameMode`/`gm_801A4014` state machine or `gm_801A4D34` scene-frame loop. A loading or playable match could therefore never appear even if every asset loaded correctly. v3.67 removes that terminal path. After the native CSS/SSS bridge, execution now resumes the original mode state machine, which performs state preload, mode callback, `GameScene::on_enter`, the retail frame loop, scene exit and the next state/mode transition. Modes outside the specialized CSS/SSS bridge dispatch through the complete `gm_GetAllGameModes()` table instead of returning an unsupported frontier. SSS Back also follows the original callback's next-state request back to CSS; it is no longer misread as a numeric outer-loop result.

Major-mode initialization now follows the retail lifecycle. After the original HSD, GX, DVD/ARQ, audio, SisLib and `gmMainLib_8015FBA4` initialization completes, `gm_801A3EF4()` initializes every linked GameMode once, as `gm_801A4510()` does in the GameCube entry point. Per-screen lazy initialization was removed from the full gameplay profile. Runtime telemetry now identifies the renderer and lifecycle as `RENDER_BACKEND name=vitaGL scene_loop=original_GameMode menu=mnMain_native`, reports `GAME_MODE_TABLE_INIT_PASS`, uses the correct `GM_TRAINING(28)` route name, and marks every retail preload/on-enter/frame-loop boundary.

The Stage Select fix is typed rather than permissive. `vita/hsd_native.c` now builds a native ARM32 `HSD_ShapeSetDesc`, native vertex/normal descriptors and native pointer arrays for every shape, while retaining the original bounded GameCube index byte streams. `pobj.c` decodes the big-endian u16, s16 and f32 vertex/normal data when the original HSD CPU shape blender consumes it on little-endian ARM. Unsupported PObj unions still fail closed. The exact set-10 graph from `MnSlMap.usd` now nativeizes, relocates, loads through original `HSD_JObjLoadJoint()` and releases cleanly; success telemetry is `GAME_SSS_NATIVE_PREPARE_PASS ... shapeanim=native`.

The missing main-menu text, Character Select difficulty label and character name all share the SIS render path. The v3.66 log proves glyph geometry and the extracted retail atlas exist, so v3.67 fixes the common camera/projection layer instead of adding replacement labels. Each Vita `HSD_Text` now retains the exact camera supplied when its SIS context is created. Capture uses that camera directly rather than selecting the first camera with a matching, non-unique GX-link mask. The SIS path without a GObj also used `MTXOrtho` to write a 4x4 matrix into a 3x4 `Mtx` and submitted it as perspective; it now uses a real `Mtx44` and `GX_ORTHOGRAPHIC`. These changes are source- and build-proven, but their visual result remains pending on a physical Vita.

Menu/CSS/SSS captures now also report a deterministic in-process `capture_sig`. Main-menu samples at frames 1, 60 and 180 include `changed_initial` and `changed_sample`, so the next log distinguishes a running HSD animation with bad presentation from a genuinely static captured graph. This measurement hashes the current commands, matrices, material/texture references and vertices; it does not substitute an animation or alter timing state.

The current authoritative profile is `MELEE_VITA_FULL_GAMEPLAY_SCENE=ON`, `MELEE_VITA_SKIP_CLASSIC_INTRO=OFF`; no Classic intro/loading bypass is enabled. vitaGL remains the only Vita renderer. Original HSD/GX code produces commands through the compatibility capture layer and vitaGL replays them; this is a native runtime port, not a diagnostic redraw of the menus.

Validation on the final v3.67 linked ARM ELF:

- full ARM compile/link, VELF, SELF and VPK packaging: PASS;
- `test_boot_arm.py`: original bootstrap plus initial SFX banks and all original major-mode `on_init` callbacks: PASS;
- `test_sss_shape_arm.py`: real `MnSlMap.usd` set 10, two native ShapeSet descriptors, original HSD load/remove: PASS;
- `test_stage_shape.py`: existing `GrNSr` ShapeSet regression remains green;
- `test_sis_font_arm.py`: the linked 146,944-byte SIS atlas exactly matches `main.dol`, SHA-256 `e8d43d10b000e59a64653e8a1b7726bdf95f9102d3ee3307ed56d7c53cde05c1`;
- deferred DVD/ARQ ordering and the 45-mode/51-event menu-boundary regressions: PASS;
- `git diff --check` and VPK ZIP integrity: PASS. Existing enum-size linker warnings remain non-fatal and are not treated as hardware proof.

Hardware artifact: `build/vita-full/SmashMeleeVita-v3.67-original-gamemode-sss-shapeset-sis.vpk`, 3,283,543 bytes, SHA-256 `a09464f815e5cc40655be63b91327cab38ada4e1384f097d97a8276a9ee9b59f`. It is byte-for-byte identical to the generated `SmashMeleeVita-assets.vpk`. Packaged `eboot.bin` is 2,852,962 bytes, SHA-256 `28646419904e7d47fdfe6e60a5a4f067aff72601b6913733436937f7062f8eea`; unstripped `melee_vita` is 9,461,232 bytes, SHA-256 `f7efba101e352ffca7daf616ed6582578e36f525d3df34bcb2c044a05526846b`; SFO is 912 bytes, SHA-256 `4dc5eb991504e8c1947ba72cdba26bd1c7049415cd8a04085d2b797d026c4088`, and reports APP_VER `00.77`.

Next physical test: install exactly the named v3.67 VPK and repeat Main Menu -> 1P -> Training -> character -> Start -> stage -> Start. The old `GAME_SSS_NATIVE_FAIL ... value=00009000` and `mnstagesel.c:461` panic must disappear. `GAME_MENU_LIVE_PASS` should show whether capture signatures change across frames. Stage Select should emit `GAME_SSS_NATIVE_PREPARE_PASS ... shapeanim=native` and `GAME_SSS_CAPTURE_INIT_PASS`; after confirmation the log must reach `GAME_MODE_RETAIL_CONTINUE_BEGIN`, `VITA_RETAIL_STATE_BEGIN ... scene=GS_TRAINING`, `VITA_RETAIL_SCENE_ONENTER_PASS` and at least one `VITA_RETAIL_PRESENT`. Photograph the main-menu text, CSS name/difficulty, Stage Select and first loading/match frame, and return the new runtime log/core if execution stops. Until those markers and frames are observed on real hardware, v3.67 is a structurally corrected first-match candidate rather than a claim of a playable port.


## 2026-09-12 — v3.58: fix the vitaGL-splash stall by completing GameCube-visible DVD/ARQ scheduling and requiring the full disc tree

The physical v3.57 run does not reach a retail scene or present. It stops after `GMMAIN_SERVICES_PASS ... stop=before_gmMainLib_final_audio_bank_init`, so the visible vitaGL splash is only the last frame on screen; the actual stall is inside `gmMainLib_8015FBA4() -> lbAudioAx_80028690() -> HSD_SynthSFXWaitForLoadCompletion()`. This is before `GMMAIN_FINAL_INIT_PASS`, title/opening entry or the new retail present bridge.

v3.57 had correctly replaced the Vita synchronous DevCom substitute with the original Melee `devcom.c` and deferred DVD/ARQ completion queues, but its scheduler had one missing GameCube interrupt invariant: `DVDGetDriveStatus()` advanced the emulated completion pump only while a DVD request was pending. Real SFX loads alternate DVD and ARQ legs. After a DVD callback submitted an ARQ leg, the retail wait path continued polling through `lb_800195D0 -> DVDGetDriveStatus()` with no DVD request left, so the ARQ completion could starve forever. v3.58 makes that scheduling boundary advance any pending emulated device work while still returning the DVD state observed on entry. `vita/tools/test_async_order_arm.py` now explicitly creates an ARQ-only phase and requires it to drain through that retail DVD-status boundary; it passes with `PASS GC async ordering: DVD and ARQ completions are deferred; ARQ-only DevCom legs advance through the retail DVD status scheduling boundary without deadlock`.

The same investigation exposed a second, independent deployment requirement. The runtime DVD adapter resolves original paths under `ux0:data/SmashMeleeVita/files/`; the VPK intentionally contains code only. The old menu/boot data ZIP supplied only a handful of top-level archives and does not contain the retail `audio/` hierarchy. During final init Melee requests `/audio/main.ssm` (or the localized `audio/us/` tree when `/usa.ini` is present). Without the complete extracted directory the request resolves to `-1`, DevCom raises its global I/O error flag and intentionally suppresses the Synth completion callback, leaving `HSD_Synth_804D772C` nonzero forever. That produces the exact observed splash stall. The README now requires a recursive copy of the complete verified `orig/GALE01/files/` tree, and `prepare_assets.py --full` / `make full-assets` can build a local hash-verified transfer ZIP while keeping copyrighted game data out of the VPK and Git.

The boot preflight itself was strengthened so this class of failure is no longer hardware-only. `test_boot_arm.py` now accepts the selected linked ELF and mounts the real extracted disc tree through `arm_disc.py`; `arm-boot-check` is mandatory in `endian-preflight`. On the v3.58 linked binary it executes the original HSD/GX/audio bring-up, the real initial SFX-bank read/DevCom chain, `gmMainLib_8015FBA4`, and continues into GM_BOOT/GS_MEMCARD. The decisive result is `ARM gmmain final init: PASS gmMainLib_8015FBA4 + initial SFX bank load completed from mounted disc`, followed by `ARM GM_BOOT enter: PASS bootOnLoad -> GS_MEMCARD -> GM_OPENING_MV`.

The complete v3.58 preflight is green: full boot/SFX, deferred DVD/ARQ ordering, GmPause, all 78 ItCo roots, Player mapping, FigaTrack semantics, fighter x48 including Samus INSTANCE, HSD amnesia/release stability, complete IfAll HUD, SceneDesc sidecars, stage-light and map-plit regressions, all 76/76 PAL stage archives, `GAMEPLAY_DAT_AUDIT_PASS` (33 fighters / 10,658 motions / 471 demos) and `FIGATREE_AUDIT_PASS` (8,011 motion chunks / 573,229 nodes / 1,220,913 tracks).

Runtime marker is `MELEE_VITA_GAME_BOOT v3.58`; VPK APP_VER is `00.68`. Final hardware artifact: `build/vita-full/SmashMeleeVita-v3.58-async-sfx-fullboot-preflight.vpk`, 3,233,983 bytes, SHA-256 `efbaddf9fc89644540c263595965390147f244d331e861dbc639857dffb24c00`. `eboot.bin` is 2,803,083 bytes, SHA-256 `bf1e558baa3b2320276d488c00e19b73234e0fe4bbd99a69bec862ba2e1636d4`; unstripped `melee_vita` is 9,291,644 bytes, SHA-256 `e7c6b55244c0fab7476d04d3567197a5e949a8457531d1131430a7bd031b2482`; SFO is 912 bytes, SHA-256 `8de1a4e20763b8421a7eb5eff78b69bc73fc70b20a447af1c96f0915dd7990ca` and contains `00.68`. The locally generated complete-disc transfer archive is `build/vita-full/SmashMeleeVita-full-disc-data.zip`, 1,165,232,563 bytes, SHA-256 `92ccdf0000a3738bbd1d189888d007b0f756a1bfac39fb5ead6b528a08658f97`; it contains 1,209 verified original disc outputs plus its manifest, passes `ZipFile.testzip()`, and explicitly contains `usa.ini`, `audio/main.ssm`, `audio/smash2.sem`, `audio/us/main.ssm` and `audio/us/smash2.sem`. `git diff --check` is clean.

The next hardware test must install v3.58 and extract the complete data ZIP directly under `ux0:data/`, yielding `ux0:data/SmashMeleeVita/files/audio/main.ssm` (and the rest of the retail tree). The first hard criterion is that the old stop after `GMMAIN_SERVICES_PASS` disappears and `GMMAIN_FINAL_INIT_PASS` appears. After that the runtime should enter the opening/title path; the vitaGL splash should be replaced once the retail render/present bridge receives its first captured frame. Any later failure is beyond this now-offline-reproduced SFX/bootstrap boundary and should be classified from its log/core rather than reopening the completed DVD/ARQ scheduling fix.


## 2026-09-12 — v3.56: replace crash-by-crash heap debugging with HSD lifecycle and complete IfAll HUD invariants

The physical-v3.55 run proves the preceding GmPause logical-name fix on real hardware: the VS-entry path now emits `VITA_PAUSE_HSD_RAW_NATIVE_PASS file=GmPause models=1 joints=12 dobjs=11 mobjs=11 pobjs=11 tobjs=11 ...` and continues beyond `fn_801A1134()`. The next stop is no longer an endian/nativeization panic. It is `memory.c:23: adr`, i.e. `HSD_MemAlloc()` received NULL from `OSAllocFromHeap()`. The supplied v3.55 core has melee_vita RX runtime base `0x8100f000` and symbolizes through `gm_Scene_Vs_OnEnter -> fn_8016E730 -> ifAll_802F390C -> ifMagnify_802FC870 -> ifMagnify_802FC3C0 -> HSD_MemAlloc`. This is the HUD/magnify setup late in VS scene construction, before the first new frame is presented.

Rather than increasing simulated RAM or patching `ifMagnify`, v3.56 closes the HSD ownership class that made this failure inevitable. Under the old `MELEE_VITA_HSD_LOAD_ONLY` profile, JObj/DObj/MObj/PObj/TObj inherited the base no-op release callback. `HSD_GObjFree()` and `HSD_JObjRemoveAll()` therefore destroyed outer class objects while nested DObj/MObj/PObj/TObj/AObj/material/TEV/envelope/ShapeSet allocations were not reclaimed inside the same scene. v3.54 had restored amnesia callbacks, which fixed stale globals across heap recreation, but that does not reclaim temporary object graphs during a live scene. v3.56 restores the original release graph for all five classes, restores `JObjInit`, retains the existing INSTANCE refcount path, and keeps genuine PTCL fail-closed.

This is guarded by a linked-ARM heap-lifecycle regression rather than hardware trial-and-error. `vita/tools/test_hsd_release_arm.py` verifies the compiled class-info callbacks (including `JObjInit`), repeatedly nativeizes/relocates/loads/removes the real PAL GmPause graph, warms the fixed-size HSD pools, then requires `OSCheckHeap()` to remain byte-for-byte stable for 24 further cycles. It passes at exactly 18,944,352 bytes free on every post-warmup cycle. An allocator failure now also emits `VITA_HSD_ALLOC_FAIL size=... heap=... free=...` before the retained assert, so any future budget failure immediately reports the requested and remaining bytes.

The VS HUD itself is now a first-class preflight boundary. `IfAll.dat` is normally preloaded as DVD type 2, where the raw hook receives `filename == NULL`; previously it had no complete raw nativeizer. v3.56 detects IfAll from its public schema and collects all ten retail DynamicModelDesc tables used by damage markers/numbers, stock, timer, nametag, magnify `lupe` and related overlays, plus the models reachable from `ScInfDmg_scene_data`. All reachable Joint/AnimJoint/MatAnimJoint/ShapeAnimJoint roots are deduplicated and converted as one graph set before relocation. `vita/tools/test_ifall_hsd_arm.py` executes the exact filename-NULL preload boundary, relocates the real PAL archive, loads/removes all 18 reachable HUD model roots, and then repeats the exact `lupe` model 32 times with a stable 18,939,488 bytes free. Result: `PASS IfAll HUD: tables=10 model_loads=18 unique_joints=18; preload filename=NULL nativeization + relocation/load/remove succeeded; lupe heap stable for 32 cycles`.

The final v3.56 linked-ARM preflight is green: GmPause exact runtime naming, ItCo 78 roots, Player mapping, filtered FigaTrack semantics, the full fighter-x48 roster including Samus INSTANCE, scene-amnesia cleanup, HSD release/heap stability, complete IfAll HUD coverage, all 76/76 PAL stage archives, the 33-fighter/10,658-motion/471-demo gameplay audit and the 8,011-chunk FigaTree audit all pass. `git diff --check` is clean. The 24 MiB reported simulated-memory value versus the 20 MiB dynamic arena was intentionally not changed: this release fixes the proven lifecycle leak instead of hiding it with a larger heap.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.56`; VPK APP_VER is `00.66`. Final hardware artifact: `build/vita-full/SmashMeleeVita-v3.56-hsd-lifecycle-ifall-preflight.vpk`, 3,225,122 bytes, SHA-256 `a7860a63a83186b696ce9866fb159c01983457a961399d7f945e2bc2be400f59`. `eboot.bin` is 2,794,341 bytes, SHA-256 `b11293b68a572bcf12e27ad94e4c5f2fa99a3999c52be1a0567d36c399159d2c`; unstripped `melee_vita` is 9,266,276 bytes, SHA-256 `c2eed38481733aa5461dcf6e07321410b8fc163bb43a09fc76eabf05df82c949`; SFO is 912 bytes, SHA-256 `a5058aa3d43c62340df9a8d722dc4955a4bfaaefef6403f58a756593ba99592b`, and contains `00.66`.

Next physical test is still Classic -> same fighter -> START, but the criterion is now subsystem-level: the old `memory.c:23 adr` stop in `ifMagnify_802FC3C0` must be gone, `VITA_IFALL_HSD_RAW_NATIVE_PASS` should appear before HUD construction, and the path should reach the first loading/match frame. If a later stop exists, allocator telemetry and the now-audited raw/lifecycle boundaries should classify it without reopening the already-covered HSD ownership and IfAll schema classes.


## 2026-09-12 — v3.55: make the GmPause raw nativeizer match the retail `lbArchive_80016DBC("GmPause", ...)` path

The physical-v3.54 run proves that the v3.53 shadow-list lifetime crash is fixed. Classic CSS exits, the selected stage reaches `VITA_STAGE_HSD_RAW_NATIVE_PASS` and `VITA_STAGE_NATIVE_PASS`, Mario plus both Ice Climbers are loaded, and Mario/Popo progress through Landing, WalkSlow, WalkMiddle, WalkFast and GuardOn FigaTrees. The old `HSD_MObjAddShadowTexture+0x14` Data Abort does not recur. The run stops later with the intentional guard `jobj.c:637: load-only JObj particle tree unsupported`.

The supplied v3.54 core localizes that guard inside the actual VS-scene enter path. `melee_vita` is loaded at RX `0x8106A000` versus linked `0x81000000` (delta `+0x6A000`); the main-thread stack at SP `0x81480B30` resolves through `HSD_JObjLoadJoint`, `fn_801A1134+0x3D`, `fn_8016E730+0x103` and `gm_Scene_Vs_OnEnter+0x0B`. `fn_801A1134()` is the pause/input-overlay setup and calls `lbArchive_80016DBC("GmPause", &scene, "ScGamPause_scene_data", 0)` before loading `scene->models[0]->joint`.

This is not a genuine particle JObj and does not require weakening the retained PTCL guard. The existing GmPause regression already proves the retail first model contains ordinary flags such as `0x20000008` which, if consumed unconverted on little-endian ARM, appear as `0x08000020` and falsely set `JOBJ_PTCL`. The real bug was a production/test naming mismatch: `mv_pause_scene_archive_prepare_raw()` only ran when `filename == "GmPause.dat"`, while the synchronous retail loader passes the logical archive name `"GmPause"`. The v3.54 hardware log correspondingly contains no `VITA_PAUSE_HSD_RAW_NATIVE_PASS` marker before the guard.

v3.55 removes the filename spelling dependency. The GmPause converter opens any candidate DAT and identifies the schema by the stable public symbol `ScGamPause_scene_data`; archives without that symbol are left untouched. This covers both the logical retail name and host/on-disc `.dat` naming, and also remains safe when the generic preloader calls the dispatcher with no filename. The output marker now tolerates a NULL filename as `?` rather than relying on a name for correctness.

`vita/tools/test_pause_raw.py` now reproduces the production boundary exactly: it invokes the linked ARM `mv_boot_archive_prepare_raw()` dispatcher with the string `"GmPause"` (not the direct converter and not `"GmPause.dat"`), then requires both false PTCL/INSTANCE flag sites to be nativeized, relocates the real archive with `HSD_ArchiveParse()`, loads `scene->models[0]->joint` through the original `HSD_JObjLoadJoint()`, and removes it successfully. It reports `PASS GmPause: 2 false ARM PTCL/INSTANCE flag(s) corrected; HSD relocation/load/remove succeeded; examples 0x2128:0x20000008->0x08000020, 0x2168:0x20000008->0x08000020`. `pause-raw-check` is now a mandatory dependency of `endian-preflight` so this exact production-name mismatch cannot silently regress.

The final v3.55 linked-ARM preflight is green: GmPause production-name dispatch, all 78 ItCo roots, Player mapping, filtered FigaTrack consumption, fighter x48 including Samus INSTANCE, HSD scene-amnesia, all 76 PAL `Gr*` archives, `GAMEPLAY_DAT_AUDIT_PASS` for 33 fighters / 10,658 motions / 471 demos and `FIGATREE_AUDIT_PASS` for 8,011 motion chunks all pass. `git diff --check` is clean.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.55`; VPK APP_VER is `00.65`. Final hardware artifact: `build/vita-full/SmashMeleeVita-v3.55-gmpause-logical-name-runtime.vpk`, 3,222,444 bytes, SHA-256 `ecae0358a23a1fe7ac4820940304ad7e9db01ef299c70cd0053a761d0ad95997`. `eboot.bin` is 2,791,863 bytes, SHA-256 `8535b4965c61788d4360913422b6d9cbe0c8fd9ac469ca26f7a433b82a2b1850`; unstripped `melee_vita` is 9,259,272 bytes, SHA-256 `e76ddc84dd9fe46f78561a52a36aac5347f0a2879a3fa682765ab81325937c0c`; SFO is 912 bytes, SHA-256 `65316f35f6158bfa8758617fee20062fa2400a37caaecd457947c105fc7b31af`, and contains `00.65`.

Next physical test: repeat Classic -> select the same fighter -> START. The decisive new runtime marker is `VITA_PAUSE_HSD_RAW_NATIVE_PASS file=GmPause ...`; the v3.54 `jobj.c:637` false-PTCL guard must disappear. The next meaningful milestone is progression beyond `fn_801A1134()` into the remainder of `gm_Scene_Vs_OnEnter` and the first loading/match frame.

## 2026-09-12 — v3.54: clear stale HSD scene globals across heap recreation and make every PAL stage pass the linked-ARM raw preflight

The physical-v3.53 run progresses materially farther than the previous FigaTree crash. Classic CSS exits normally, Mario and Captain demo data load, the selected stage fully reaches `VITA_STAGE_HSD_RAW_NATIVE_PASS` and `VITA_STAGE_NATIVE_PASS`, stage lighting selects map 30, and Mario proceeds through Landing / WalkSlow / WalkMiddle / WalkFast / GuardOn FigaTrees. The display still shows the last CSS frame because the process dies during match-scene construction before a new scene present; stage loading itself is no longer the blocker.

The supplied core is a main-thread Data Abort in the exact v3.53 ELF. Its `melee_vita` RX runtime base is `0x81014000` versus linked `0x81000000` (delta `+0x14000`), while the RW runtime base is `0x81400000` versus linked `0x81380000` (delta `+0x80000`). Runtime PC `0x8122BAB4` resolves to linked `0x81217AB4 = HSD_MObjAddShadowTexture+0x14`; runtime LR `0x810B8B41` resolves inside `lbShadow_8000ED54`, the fighter-shadow setup path. At the fault R0 is a freshly created valid shadow TObj, R2 is runtime `tobj_shadows` (the linked global at `0x8176B510` plus the RW relocation), and traversal has loaded R3=`0x0000A631` before executing `ldr r3,[r3,#8]`. The crash is therefore a stale linked-list pointer, not another stage/Figa/endian failure.

The lifetime bug came from the Vita `MELEE_VITA_HSD_LOAD_ONLY` class setup. `HSD_CreateMainHeap()` calls `hsdForgetClassLibrary("sysdolphin_base_library")` before destroying/recreating the scene heap; the hardware log confirms that boundary with `GAME_CSS_GOBJ_HEAP_SYNC generation=2`. Upstream HSD relies on class `amnesia` callbacks at this point. In particular `MObjAmnesia()` clears `tobj_toon` and `tobj_shadows`. The Vita load-only branches had disabled those callbacks together with ownership/release callbacks, so a CSS/intro shadow texture list could survive while its backing heap was recycled. The next fighter then traversed freed scene memory in `HSD_MObjAddShadowTexture()`.

v3.54 keeps the existing Vita load-only release/ownership policy but restores the safe upstream `amnesia` callbacks for JObj, DObj, MObj, PObj and TObj. This clears scene-owned global/cache state (including `current_jobj`, MObj shadow/toon heads, PObj vertex caches and TObj list state) whenever the class library is forgotten, without enabling release paths whose ownership adapters are still intentionally restricted. A linked-ARM regression, `vita/tools/test_hsd_amnesia_arm.py`, verifies all five function pointers in the compiled class infos, seeds non-null `tobj_shadows`/`tobj_toon`, invokes the real ARM `hsdForgetClassLibrary()`, and requires both heads to become NULL. It reports: `PASS HSD scene amnesia: JObj/DObj/MObj/PObj/TObj callbacks registered; MObj shadow/toon heads cleared across class-library forget`.

The same preventive stage sweep also closed the remaining raw-stage gaps. `GrSt` legitimately uses a linear spline (type 0) with NULL `segPoly`; the validator/converter now requires polynomial coefficients only for curved spline types 1-3. `GrNSr` uses retail `POBJ_SHAPEANIM`, so the raw/native paths now support the bounded `HSD_ShapeSetDesc` layout and the original Vita HSD loader accepts ShapeSet-backed PObjs. Pokémon Stadium's `GrPs*` family uses HSD extern chains whose terminal word is `0xFFFFFFFF`; `MvDat` now distinguishes external reference fields from ordinary relocations so they are left intact for `HSD_ArchiveLocateExtern()`. Finally, `GrOp` exposed a local nativeizer bookkeeping bug: `stage_hsd_raw_pobj()` read a valid SKIN `u.joint`, then reused the same `target` variable for `verts`; after the ShapeSet-order change it could therefore walk the VtxDesc pointer as a JObj. v3.54 preserves a separate `union_target`. The complete linked-ARM `test_stage_raw.py` sweep is now mandatory in `endian-preflight` and reports 76/76 PAL `Gr*` archives PASS, zero failures.

The final v3.54 preflight is green on the linked ARM binary: all 78 ItCo roots relocate/load/remove through original HSD; Player mapping, filtered FigaTrack consumption, fighter x48 (including Samus' genuine INSTANCE accessory), HSD scene-amnesia and all 76 stage archives pass; `GAMEPLAY_DAT_AUDIT_PASS` still covers 33 fighters / 10,658 motions / 471 demos; `FIGATREE_AUDIT_PASS` covers 8,011 motion chunks; `GmPause` still relocates/loads/removes successfully; and `git diff --check` is clean.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.54`; VPK APP_VER is `00.64`. Final hardware artifact: `build/vita-full/SmashMeleeVita-v3.54-hsd-scene-amnesia-stage-preflight.vpk`, 3,222,933 bytes, SHA-256 `cb28130db93a6155e8a5624f6aee3f4a61ef7f37648991bd9f1ba2ac389e26d7`. `eboot.bin` is 2,792,027 bytes, SHA-256 `ed6ad6ec3cb25e5638ef80daf84e1a73dcbed42b9e81076347cf7af4ff957955`; unstripped `melee_vita` is 9,259,296 bytes, SHA-256 `a60f80bf1854905533e9d093e9ebd0f3f7801f240af1974ffd17c3e4a9410223`; SFO is 912 bytes, SHA-256 `a47b62f9f3b1e759ff8589277911d7ab46eb0098dff394a36ba6429c74c88d1f`, and contains `00.64`.

Next physical test: repeat Classic -> select the same fighter -> START. The v3.53 crash in `HSD_MObjAddShadowTexture+0x14` must disappear; the meaningful milestone is progression beyond the Mario GuardOn FigaTree and fighter-shadow setup into the first actual match/loading/render marker. Because the previous display remained on CSS only because no new frame had been presented before the fault, seeing any loading/match frame is strong evidence that this scene-lifetime blocker is gone.


## 2026-09-11 — v3.53: fix retail FigaTrack filtered-channel consumption in the ARM runtime

The physical-v3.52 run proves the preceding endian/layout fixes and advances materially farther. It reaches the Classic Easy intro, nativeizes/loads Mario, then nativeizes Popo and emits `VITA_FIGATREE_NATIVE_PASS symbol=PlyPopo5K_Share_ACTION_IntroR_figatree nodes=50 tracks=114 type=0 flags=00000000 frames=10.000000` before the process stops. The supplied core reports a main-thread Data Abort (`stop_reason=0x30004`). With the dump's `melee_vita` RX relocation (`0x81071000` runtime versus `0x81000000` linked), PC `0x811a66c0` resolves to `fn_8001E60C+0x60` and LR `0x811a6814/15` to `lbAnim_8001E7E8+0x44`; the stack continues through `ftAnim_8006FCE4`, `ftAnim_8006EBE8`, `Fighter_ChangeMotionState`, `ftCo_800BF108` and `ftDemo_CreateFighter`. This confirms that v3.52's `Player_80036E20` mapping fix is passed and the new failure is inside fighter-animation construction.

The crash is not another byte-order failure. Core memory contains a valid already-nativeized Popo IntroR track group whose first nine `FigaTrack.obj_type` values are `7,6,5,8,9,10,1,2,3`; the `frames` argument to `fn_8001E60C()` is 9. The decompiled C advanced `track` only inside the branch that creates an `HSD_FObj`, while types 5/6/7 are deliberately filtered by that branch. A group beginning with type 7 therefore reread the same record for every iteration, and a group containing only filtered records left local `fobj` uninitialized before the unconditional `fobj->next = NULL`. The retail data proves those cases are normal rather than malformed input.

v3.53 corrects this semantic mismatch on Vita without disturbing the matching PPC/upstream path: under `MELEE_VITA_PLATFORM`, `fn_8001E60C()` consumes one `FigaTrack` on every loop iteration, allocates/links an `HSD_FObj` only for non-5/6/7 records, and terminates the list only when `prev != NULL`. A complete PAL scan shows why this must be a class fix rather than a Popo special case: across 565,192 Figa node groups, 65,510 contain filtered types 5/6/7, 20,842 begin with one of them, and 17,215 are composed entirely of filtered records. The existing Figa schema audit still covers all 33 fighters, 8,011 non-empty motion mini-DATs and 26 demo-AJ files (`573,229` node bytes / `1,220,913` tracks).

A linked-ARM regression, `vita/tools/test_lbanim_figatrack_arm.py`, hooks only `HSD_FObjAlloc()` and executes the real compiled `fn_8001E60C()`. It reproduces the hardware pattern `7,6,5,8,9,10,1,2,3` and requires exactly six generated FObjs, then exercises an all-filtered `5,6,7` group and requires zero allocations plus a NULL result. It reports `PASS lbAnim FigaTrack: filtered 5/6/7 tracks advance correctly and filtered-only groups return NULL`. This test is now part of `endian-preflight` alongside the ItCo/HSD and Player mapping ARM regressions. The final preflight is green: all 78 ItCo item-model roots relocate/load/remove through the original HSD runtime, Player mapping is independent of PPC `.data` adjacency, the FigaTrack regression passes, `GAMEPLAY_DAT_AUDIT_PASS` covers 33 fighters / 10,658 motions / 471 demos, `FIGATREE_AUDIT_PASS` covers all 8,011 motion chunks, the GmPause regression remains green, and `git diff --check` passes.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.53`; VPK APP_VER is `00.63`. Final hardware artifact: `build/vita-full/SmashMeleeVita-v3.53-figatrack-filter-runtime.vpk`, 3,218,444 bytes, SHA-256 `b7aab28a59effc5c6b8a341c17ed86b319b76424560a989cc280c0a74169b8c6`. `eboot.bin` is 2,787,526 bytes, SHA-256 `56c42dee0b636ea6cd189ffaa42dc9f42abf4bdb0338bb02fce138df6665cc2d`; unstripped `melee_vita` is 9,246,212 bytes, SHA-256 `4725a58c38604e91bb72d2a99971963ae2c1d8d69e70f347994fade5273e0d69`; SFO is 912 bytes, SHA-256 `32577eeff195cf8e7b90e5f60c199d4bedbe02cdc23fbecaf39e6b2ff9eb78f0` and contains `00.63`.

Next physical test: repeat Classic -> Mario -> START. The v3.52 Data Abort in `fn_8001E60C()` must disappear after the Popo `PlyPopo5K_Share_ACTION_IntroR_figatree` marker. The meaningful next milestone is completion of both demo-fighter motion-state setups and progression beyond `ftDemo_CreateFighter`; any later stop should be classified from its core before changing the audited Figa endian/nativeization layer.

## 2026-09-11 — v3.52: systemic PPC big-endian preflight + remove the Player `.data` adjacency trap

The physical-v3.51 run validates the new batch ItCo path on real hardware before the next failure: it emits `VITA_HSD_GRAPH_SET_NATIVE_PASS ... joint_roots=78 ... joints=1663 ...` followed by `VITA_ITCO_NATIVE_PASS ... articles=94 ... models=92 model_joints=78`, and then continues through the effect/intro/common-fighter nativeizers to `VITA_PLCO_NATIVE_PASS`. The supplied core is a hard stop rather than another HSD endian assertion. After applying the module relocation recorded by the dump, the active PC resolves to `Player_80036E20+0x1d` and LR to `ftDemo_SetArchiveData+0x62`; the stack continues through the Classic Easy intro scene.

That stop exposed a separate PPC portability assumption in `player.c`: five functions cast the address of the string `str_PdPmdat_start_of_data` to a synthetic struct and expected `ftMapping_list` to live immediately after the surrounding original `.data` objects. The Vita linker does not preserve that GameCube object adjacency (`ftMapping_list` is before the two strings), so Mario's CharacterKind indexed unrelated memory and supplied a bogus fighter/demo mapping to `ftDemo_SetArchiveData`. v3.52 removes the synthetic aggregate and makes all five users access `ftMapping_list` directly (`internal_id`, `extra_internal_id`, `has_transformation`). A linked ARM regression now checks Mario, Fox, Ice Climbers and the Zelda/Sheik transform pair and reports `PASS player mapping: Player_80036E20 uses ftMapping_list, independent of PPC .data adjacency`.

The endian work is no longer driven by physical crash iteration. `ItCo.dat`/`ItCo.usd` now discover every reachable `ItemModelDesc::x0_joint` before relocation and nativeize the complete deduplicated model forest. PAL contains 78 unique item-model JObj roots plus 16 reachable RObj descriptors per variant; those RObjs are all retail `REFTYPE_BYTECODE`, so v3.52 includes typed pre-relocation conversion for `HSD_RObjDesc`, `HSD_ByteCodeExpDesc` and `HSD_RvalueList` plus the corresponding load-only bytecode-RObj adapter. Other RObj types remain fail-closed. The batch validator and raw HSD walker both use open-addressed hash visited/index tables instead of O(n^2) linear scans, making the full ItCo conversion practical on ARM/Vita.

The other production bypass was the fighter FigaTree path: the two direct `HSD_ArchiveParse()` calls in `ftdata.c` parse copied mini-DAT motion chunks without going through `lbArchive`'s raw dispatcher. They now call `mv_fighter_figatree_prepare_raw()` first, converting the typed FigaTree/FigaTrack scalar fields while leaving node/script byte streams untouched. The permanent PAL audit covers all 33 fighters, 8,011 non-empty motion chunks and 26 demo-AJ files (`573,229` node bytes and `1,220,913` tracks). A source-level loader census confirms every gameplay DAT/HSD parse boundary is now covered before relocation: synchronous `lbArchive_*`, preload DVD type 2, async effects type 3, stage type 4/dynamic stage archives, and both fighter mini-DAT parses. The remaining direct archive parses are probe/test code rather than retail gameplay loaders.

The final v3.52 `endian-preflight` is green on the linked ARM binary and the complete PAL asset set. The ARM ItCo regression reports `articles=94 models=92 joint_roots=78 loaded=78` and successfully performs raw conversion, real HSD relocation, RObj bytecode reference resolution, `HSD_JObjLoadJoint()` and removal for all roots. `GAMEPLAY_DAT_AUDIT_PASS` remains green for 33 fighters / 10,658 motions / 471 demos and both ItCo variants; `FIGATREE_AUDIT_PASS` remains green for all 8,011 motion chunks. Hardware is therefore no longer the primary detector for the known DAT/HSD/FigaTree big-endian schemas. This does not claim that an unrelated bespoke binary format outside those audited parse boundaries can never contain another PPC-endian field.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.52`; VPK APP_VER is `00.62`. Final hardware artifact: `build/vita-full/SmashMeleeVita-v3.52-systemic-endian-layout.vpk`, 3,218,772 bytes, SHA-256 `7a0875aa2e46605d39e8e85606eb0be49683255326251007495095443a350799`. `eboot.bin` is 2,787,663 bytes, SHA-256 `5526eaab72a44c371ec832bab177987ad542c6ba4268f826c52d6bf93feb424e`; unstripped `melee_vita` is 9,246,228 bytes, SHA-256 `48c2d47e8f34e643f527d8a0bb1d234c5c5f26db436d0c63dee181ee06a419ce`; SFO is 912 bytes, SHA-256 `6274588fa25521ffd9203c0374d5b93e14183e9cf325b66a8616c9fa487eeff1` and contains `00.62`.

Next physical test: use the same Classic -> Mario -> START route. The v3.51 `Player_80036E20`/`ftDemo_SetArchiveData` hard crash should be gone. The existing ItCo batch marker should still appear; after `VITA_PLCO_NATIVE_PASS` the run should advance into the Classic Easy demo setup instead of stopping in the bogus mapping lookup. Any later failure should now be classified first as an ordinary PPC/ARM portability/runtime issue unless an audited raw-boundary invariant explicitly fails.

## 2026-09-11 — v3.51: nativeize the GmPause SceneDesc HSD graph before relocation

The physical-v3.50 run proves the preventive PlCo/common-fighter pass on hardware: special-parts, ColorOverlay commands, model-shift vectors, CPU data, Mario/Samus fighter graphs, common/menu effects and the selected stage all progress through their nativeization markers. The run then stops deliberately in `JObjLoad()` at `jobj.c:636` with `load-only JObj instance/particle tree unsupported`. This is another fail-closed HSD guard rather than a random ARM/GPU fault.

The matching PAL asset is `GmPause.dat`, which is loaded as an ordinary `SceneDesc` through `lbArchive_80016DBC("GmPause", ..., "ScGamPause_scene_data", ...)`. That path was outside the existing stage/fighter/effect HSD raw nativeizers. Its first model tree contains two JObjs at data offsets `0x2128` and `0x2168` with valid retail flags `0x20000008`; the same four bytes, when consumed unconverted as little-endian ARM scalars, become `0x08000020` and falsely set `JOBJ_PTCL`. The asset contains no genuine `JOBJ_PTCL` or `JOBJ_INSTANCE` node in this model tree, so weakening the runtime ownership guard would have fixed the symptom at the wrong layer.

v3.51 adds `mv_pause_scene_archive_prepare_raw()` to the common pre-relocation archive dispatcher. For `GmPause.dat` it resolves `ScGamPause_scene_data`, walks the `DynamicModelDesc**` model table and nativeizes each JObj plus its AnimJoint, MatAnimJoint and ShapeAnimJoint graphs using the existing typed HSD raw walkers before `HSD_ArchiveParse()` relocates pointer fields. Genuine particle/instance unions, RObjs and other unsupported layouts still fail closed. Successful hardware execution will emit `VITA_PAUSE_HSD_RAW_NATIVE_PASS`.

A permanent ARM regression, `vita/tools/test_pause_raw.py`, uses the original PAL `GmPause.dat`, proves both false ARM PTCL flags (`0x2128: 0x20000008 -> 0x08000020`, `0x2168: 0x20000008 -> 0x08000020`), runs the new raw converter in the linked ARM binary, then executes the real `HSD_ArchiveParse()`, `HSD_JObjLoadJoint()` and `HSD_JObjRemoveAll()` chain successfully. Result: `PASS GmPause: 2 false ARM PTCL/INSTANCE flag(s) corrected; HSD relocation/load/remove succeeded`. The full v3.50 gameplay-DAT audit also remains green with all 33 fighters, 10,658 motions, 471 demos, the complete PlCo pass and both ItCo archives.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.51`; VPK APP_VER is `00.61`. The authoritative main checkout builds through ARM compile/link, VELF, SELF and VPK with exit 0; `git diff --check` passes. Hardware artifact: `build/vita-full/SmashMeleeVita-v3.51-gmpause-scene-hsd.vpk`, 3,214,302 bytes, SHA-256 `30aa950bb2a737841d5f22aedb3043909dfe99255a86af0eeca8695315351511`; it is byte-for-byte identical to the generated `SmashMeleeVita-assets.vpk`. `eboot.bin` is 2,783,638 bytes with SHA-256 `4d9f52032410d5f5ae1be4b5cce60b664e7eddee3832b4b9b7cd7dd56c1eb1f1`. The unstripped `melee_vita` ELF is 9,238,308 bytes with SHA-256 `85ebbead4abfac193e89ab0736dfa5b9f258f28195d3201a4cf6e7baf91a3958`. The packaged SFO is 912 bytes, SHA-256 `e53a224939a583513266875f47afcd53d35e56ae5765de9d75b4a6434f0d3b5a`, and reports APP_VER `00.61`.

Next physical test: Classic -> select character -> START and follow the same route as v3.50. The decisive new marker is `VITA_PAUSE_HSD_RAW_NATIVE_PASS file=GmPause.dat ...`; the old `jobj.c:636: load-only JObj instance/particle tree unsupported` stop must disappear. Any later stop is the next genuine retail gameplay/HSD frontier and should be diagnosed without relaxing the retained instance/particle ownership guards.

## 2026-09-11 — v3.50: preventive PlCo PPC/ARM ABI sweep before the next hardware round-trip

The physical-v3.49 run advances beyond the live HSD display callback failure and reaches fighter construction far enough to report `fighter parts num not match! player 1`. Rather than treating that as another isolated crash, v3.50 performs a preventive audit of the remaining `PlCo.dat` common-fighter data consumed by the immediate gameplay path. The strongest direct explanation for the v3.49 stop is `ftLoadCommonData` `pData[5]`: the reachable special-parts record contains retail count 13 (`00 00 00 0D`), which an unconverted little-endian ARM load would interpret as `0x0D000000`.

The same audit found several additional common-data families that had not yet been nativeized but are deterministic from the PAL retail schema. v3.50 converts the `0x138` item-throw attribute table, all four reachable special-parts records (16 total entries), three model-shift pointer/count records with 13 total `Vec2` values, bunnyhood/metal/gravity-weight modifier tables, the common accessory/trophy/common HSD graphs, and the CPU attack database. Pointer words remain relocation-managed; only typed scalar payloads are converted. The CPU pass distinguishes the byte-oriented command-script table from numeric attack data: the scripts remain byte-identical while 189 unique `ftCo_AttackEntry` lists containing 947 scalar records, the 32 fighter-reach floats and six weapon-reach floats are nativeized.

The ColorOverlay command streams required a separate PPC-to-ARM bitfield conversion rather than a byte swap. The permanent PAL audit walks the complete reachable graph and finds 1,211 commands with only opcodes `0,3,4,5,6,7,10-19,21-23`. v3.50 decodes the PPC MSB-first fields and explicitly repacks them for ARM LSB-first allocation. Relocation-backed subroutine/goto targets remain untouched and are followed recursively; byte-oriented colors stay byte-oriented; the verified GFX, SFX and fighter-command payload layouts are converted field-by-field. Malformed or unknown layouts still fail closed.

The common HSD roots exposed another preventive boundary. `PlCo.pData[8]` contains a JObj plus an AnimJoint root, while `pData[16]` and `pData[20]` are additional JObj graphs. The existing typed JObj raw nativeizer is reused, and v3.50 adds `mv_hsd_anim_graph_prepare_raw()` so common AnimJoint/AObj/FObj scalars are validated and nativeized before HSD relocation. Success telemetry includes `VITA_HSD_ANIM_RAW_NATIVE_PASS`. The roster-wide fighter `ftData.x5C` audit remains intentionally conservative and green across 30 non-null / three null graphs (2,136 JObjs, 331 DObjs, 36 MObjs, 779 PObjs and 72 TObjs); therefore no speculative `JOBJ_INSTANCE`, `JOBJ_PTCL`, RObj or shape-PObj ownership adapters were added for this release.

The expanded PAL preflight passes with: `GAMEPLAY_DAT_AUDIT_PASS fighters=33 motions=10658 demos=471 costume_tobj_arrays=28 costume_tobj_indices=58 vis_lookups=126 vis_temp_arrays=202 vis_temp_entries=309 vis_byte_indices=2692 fighter_parts_hsd=30/3 fighter_parts_nodes=2136/331/36/779/72 plco=1 plco_special_parts=4/16 plco_model_shift_vecs=13 plco_common_hsd=9/1/1 plco_common_anim=9 plco_color_cmds=1211[0:1,3:89,4:89,5:44,6:8,7:46,10:79,11:345,12:74,13:23,14:1,15:10,16:11,17:2,18:178,19:65,21:88,22:12,23:46] plco_cpu_attacks=189/947 itco=2 articles=94/94 hurts=24/24 dynamics=3/3`. Both the managed worktree and authoritative source checkout build through ARM compile/link, VELF, SELF and VPK with exit 0; `git diff --check` also passes. The existing linker enum-size warnings remain, but all port targets already compile with `-fno-short-enums`, so v3.50 does not introduce an unverified global ABI-flag change.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.50`; VPK APP_VER is `00.60`. Hardware artifact: `build/vita-full/SmashMeleeVita-v3.50-plco-preflight-abi.vpk`, 3,214,074 bytes, SHA-256 `cfc94e7db3bf423f7a9044040fcb02fdd465e70645406cd93406e529c6bb8eb6`; it is byte-for-byte identical to the freshly generated `SmashMeleeVita-assets.vpk`. `eboot.bin` is 2,783,046 bytes with SHA-256 `3947a7f53266471abdec757a322d384d9d01aa705f96e15ec6f165ba23836c4f`. The unstripped `melee_vita` ELF is 9,237,300 bytes with SHA-256 `73051ac3d0da232a664531082544aa6b3ee3c935ea29de3002f51fdbfbd7b665` and contains the v3.50/upstream markers plus `VITA_PLCO_SPECIAL_PARTS_NATIVE_PASS`, `VITA_PLCO_MODEL_SHIFT_NATIVE_PASS`, `VITA_PLCO_COLOR_COMMAND_NATIVE_PASS`, `VITA_PLCO_CPU_NATIVE_PASS` and `VITA_HSD_ANIM_RAW_NATIVE_PASS`. The packaged SFO is 912 bytes, has valid `\0PSF` magic, SHA-256 `040977fcc689c067e89410b642f5a703d8197656646e6689b61b1b826169a728`, and reports APP_VER `00.60`.

Next physical test: Classic -> select character -> START. Expected new proof while `PlCo.dat` loads is `VITA_PLCO_SPECIAL_PARTS_NATIVE_PASS records=4 entries=16`, `VITA_PLCO_MODEL_SHIFT_NATIVE_PASS vectors=13`, `VITA_PLCO_COLOR_COMMAND_NATIVE_PASS commands=1211 ...`, `VITA_PLCO_CPU_NATIVE_PASS lists=189 entries=947 ...`, and `VITA_HSD_ANIM_RAW_NATIVE_PASS ...`. If the v3.49 diagnosis is correct, `fighter parts num not match! player 1` should disappear. Physical validation is still pending, and this release does not claim that every fighter action-script format or every unsupported stage/HSD ownership mode has been solved.

## 2026-09-11 — v3.49: enable the authentic HSD live-display method chain on Vita

The physical-v3.48 run proves the fighter visibility nativeizer on hardware. Both Mario and Captain advance through `VITA_FIGHTER_PARTS_HSD_RAW_NATIVE_PASS`, `VITA_FIGHTER_VIS_NATIVE_PASS`, `VITA_FIGHTER_TOBJ_INDEX_NATIVE_PASS` and `VITA_FIGHTER_DATA_NATIVE_PASS`; the previous v3.47 Data Abort in `ftParts_80074D7C()` is gone. The run proceeds through both fighter HSD model loads and into the live display path of the restored `GS_INTRO_EASY` scene.

The supplied v3.48 core is a new failure class: main-thread `stop_reason=0x30003`, `PC=0`, IFAR/DFAR zero, with LR `0x8123740f`. Using the module RX base recorded in that same core (`0x81027000`) gives linked LR `0x8121040f`, which resolves to `HSD_JObjDispDObj+0x3b`. At the fault, R0 is the active JObj, R1 the viewing matrix and R2 the destination position-matrix buffer, matching the indirect call `HSD_JOBJ_METHOD(jobj)->make_pmtx(jobj, vmtx, mtx)`. The target function pointer is null.

The null callback is not corrupt DAT data. `MELEE_VITA_HSD_LOAD_ONLY` was originally designed while Vita rendered only through the earlier capture/viewer path; under that profile `JObjInfoInit()` installed only `load` and `make_mtx`, leaving `make_pmtx` and `disp` null. A systematic class-info audit found the same issue farther down the live display chain: load-only also omitted `DObj.disp`, `PObj.disp/setup_mtx`, `MObj.setup/unset/make_texp/setup_tev`, and `TObj.make_texp/make_mtx`. `HSD_MObjLoadDesc()` also skipped `HSD_MObjCompileTev()`, which would have made the next material setup fail even after fixing the first callback.

v3.49 keeps the Vita load-only ownership/release policy but enables the authentic render methods required by retail scene drawing. JObj now exposes `HSD_JObjMakePositionMtx` and `HSD_JObjDispSub`; DObj exposes `HSD_DObjDisp`; PObj exposes `HSD_PObjDisp` and `PObjSetupMtx`; MObj exposes setup/unset/TEV methods and compiles TEV descriptors on Vita; TObj exposes its texture-matrix and TExp builders. This does not enable the unadapted release/RObj/instance ownership paths. GX calls continue through the existing Vita capture/backend implementation.

The full PAL gameplay-DAT audit remains green after the display-path change: `GAMEPLAY_DAT_AUDIT_PASS fighters=33 motions=10658 demos=471 costume_tobj_arrays=28 costume_tobj_indices=58 vis_lookups=126 vis_temp_arrays=202 vis_temp_entries=309 vis_byte_indices=2692 fighter_parts_hsd=30/3 fighter_parts_nodes=2136/331/36/779/72 plco=1 itco=2 articles=94/94 hurts=24/24 dynamics=3/3`. `MELEE_VITA_SKIP_CLASSIC_INTRO=OFF` remains in the authoritative cache.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.49`; VPK APP_VER is `00.59`. Managed and authoritative builds both complete through ARM compile/link, VELF, SELF and VPK with exit 0. Hardware artifact: `build/vita-full/SmashMeleeVita-v3.49-hsd-live-display.vpk`, 3,209,213 bytes, SHA-256 `1b402bb70755a5c46dcf43c0ee8b1157a528b597199ec18abafef6d76e1c5292`; it is byte-for-byte identical to the freshly generated `SmashMeleeVita-assets.vpk`. `eboot.bin` is 2,778,004 bytes with SHA-256 `ae52125167aca5e332628a32bf83e4e4709d23be0c08504148fe6ec1b4a7efb8`. The unstripped `melee_vita` ELF is 9,223,864 bytes with SHA-256 `cb04ade34cc9c6b24acb22430885bf26e6fcdff142432a3aac50788f65912753`. The packaged SFO is 912 bytes and reports APP_VER `00.59`.

Next physical test: Classic -> select character -> START. The v3.48 `PC=0` stop in `HSD_JObjDispDObj()` should disappear. The next desired visible milestone is the actual retail VS/loading screen; because this is the first build that enables the complete live HSD display callback chain on Vita, visual correctness still requires hardware validation.

## 2026-09-11 — v3.48: nativeize nested fighter model-visibility counts before ftParts walks them

The physical-v3.47 run proves the new `ftData.x5C` HSD boundary on hardware: `VITA_FIGHTER_PARTS_HSD_RAW_NATIVE_PASS file=? root=ftData.x5C off=0002a840 joints=61 dobjs=8 mobjs=1 pobjs=21 tobjs=2 ...` is emitted and the prior `mobj has unexpected blending flags (0x3c001000)` assertion is gone. The run then crashes later during demo-fighter part visibility setup, after `VITA_FIGHTER_HSD_RAW_NATIVE_PASS ... PlyMario5K_Share_joint ...`, with a real Data Abort rather than an HSD panic.

The supplied v3.47 core reports `stop_reason=0x30004`, `DFSR=0xC7`, `DFAR=0x01000015`, `PC=0x8126f4e6`, and `R0=0x01000001`. Using the core's actual melee module RX base (`0x8105d000`) maps the faulting PC back to `HSD_DObjSetFlags+2`; LR maps to `ftParts_80074D7C+0x61`. Core memory shows the active Mario `FtPartsVis` has `model_num=1` and valid relocated visibility pointers, but the nested scalar counts are still GameCube-endian: `FtPartsVisLookup.x0` is serialized `00 00 00 01` (retail 1, ARM sees `0x01000000`) and the first `TempS.x0` is `00 00 00 2B` (retail 43, ARM sees `0x2B000000`). The resulting loop walks past the 43 valid byte indices and eventually passes an invalid `0x01000001` value as an `HSD_DObj*`, matching the Data Abort exactly.

v3.48 extends the typed `ftData.x8` pre-relocation nativeizer through the complete visibility hierarchy `vis_table -> FtPartsVisLookup[] -> TempS[]`. Only the two serialized u32 count fields are endian-converted; relocation-managed pointers remain untouched for `HSD_ArchiveParse`, and the terminal DObj index lists remain immutable u8 byte arrays. The converter validates `model_num <= 11`, table shape/bounds, lookup counts (`<=32`), TempS counts (`<=256`), pointer/null consistency and every byte-index span, deduplicating shared lookup/temp arrays before conversion. Success emits `VITA_FIGHTER_VIS_NATIVE_PASS`; existing ftParts/HSD assertions remain enabled.

The permanent PAL audit now validates this hierarchy across all 33 fighter DATs: 126 unique visibility lookup arrays, 202 TempS arrays, 309 TempS records and 2,692 terminal byte indices. Current full result: `GAMEPLAY_DAT_AUDIT_PASS fighters=33 motions=10658 demos=471 costume_tobj_arrays=28 costume_tobj_indices=58 vis_lookups=126 vis_temp_arrays=202 vis_temp_entries=309 vis_byte_indices=2692 fighter_parts_hsd=30/3 fighter_parts_nodes=2136/331/36/779/72 plco=1 itco=2 articles=94/94 hurts=24/24 dynamics=3/3`.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.48`; VPK APP_VER is `00.58`; `MELEE_VITA_SKIP_CLASSIC_INTRO=OFF` remains active. Managed and authoritative builds both complete through ARM compile/link, VELF, SELF and VPK. Hardware artifact: `build/vita-full/SmashMeleeVita-v3.48-fighter-visibility-endian.vpk`, 3,197,276 bytes, SHA-256 `6e296a89489cfe31723f2b8f298beee07c2edaf16cf6d10061891e7eefc60a1d`; it is byte-for-byte identical to the freshly generated `SmashMeleeVita-assets.vpk`. `eboot.bin` is 2,766,177 bytes with SHA-256 `cb19dd7ac6413dcd1f32c2d1e037d90f3279e780177a449ee2b891efb7fcbd79`. The unstripped `melee_vita` ELF is 9,203,048 bytes with SHA-256 `06822d4eec5dd81f293b52f2eceab5f532df076384103cc115e5ce9a1bcb0b11`. The packaged SFO is 912 bytes and reports APP_VER `00.58`.

Next physical test: Classic -> select character -> START. Expected proof for Mario is `VITA_FIGHTER_VIS_NATIVE_PASS file=? root=ftDataMario ...` followed by the existing fighter data/HSD markers; the v3.47 `HSD_DObjSetFlags` Data Abort at `DFAR=0x01000015` should disappear. The target remains the actual retail VS/loading presentation.

## 2026-09-11 — v3.47: nativeize the fighter-parts HSD graph behind ftData.x5C

The physical-v3.46 run proves the costume-TObj u16 fix on hardware: `VITA_FIGHTER_TOBJ_INDEX_NATIVE_PASS ... root=ftDataMario count=2 arrays=1 indices=2` is emitted and the previous `can't find tobj!` assertion is gone. The restored `GS_INTRO_EASY` path then advances into demo-fighter construction and stops deliberately in `HSD_DObjLoadDesc()` with `mobj has unexpected blending flags (0x3c001000)` / `dobj.c:312`. The supplied core is again an assert/panic path rather than a GPU or Data Abort failure.

Core symbolization identifies the caller as `ft_800C85B8()` (via `ftCo_800C884C()` and `ftDemo_CreateFighter()`), which is important: this code does not load the already-nativeized `PlMrNr.dat` costume root. It walks `fp->ft_data->x5C` from the main `PlMr.dat` and calls `HSD_DObjLoadDesc(sp20->u.dobjdesc)`. That `ftData.x5C` HSD_Joint graph was still outside the gameplay nativeizer. Its retail Mario MObj descriptor stores `rendermode=0x0000003c`; on little-endian ARM the untouched bytes are observed as `0x3c000000`, and `HSD_MObjLoad()` then ORs `RENDER_TOON (0x1000)`, producing exactly the logged invalid value `0x3c001000`.

v3.47 routes every non-null `ftData.x5C` graph through the existing typed pre-relocation HSD nativeizer instead of adding an isolated rendermode swap. A new `mv_hsd_joint_graph_prepare_raw()` boundary first validates the entire graph with `mv_hsd_native_validate_raw_at()`, then nativeizes JObj/DObj/MObj/PObj/TObj, materials, vertex descriptors, images/TLUT/LOD/TEV, matrices, envelopes and splines while the DAT relocation metadata still identifies pointer fields. Failure remains fail-closed with `VITA_FIGHTER_PARTS_HSD_RAW_VALIDATE_FAIL`; success emits `VITA_FIGHTER_PARTS_HSD_RAW_NATIVE_PASS`. The original DObj blending assertion remains unchanged.

The permanent PAL gameplay audit now walks this graph family offline across all 33 fighter DATs. Thirty fighters have a non-null `x5C` graph and three are legitimately null; the verified set contains 2,136 JObjs, 331 DObjs, 36 MObjs, 779 PObjs and 72 TObjs. Current result: `GAMEPLAY_DAT_AUDIT_PASS fighters=33 motions=10658 demos=471 costume_tobj_arrays=28 costume_tobj_indices=58 fighter_parts_hsd=30/3 fighter_parts_nodes=2136/331/36/779/72 plco=1 itco=2 articles=94/94 hurts=24/24 dynamics=3/3`. The authoritative `stage_archive_vita.c` v3.41 spline/raw-validator fixes were explicitly preserved while adding the reusable x5C helper.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.47`; VPK APP_VER is `00.57`; `MELEE_VITA_SKIP_CLASSIC_INTRO=OFF` remains active. Managed and authoritative builds both complete through ARM compile/link, VELF, SELF and VPK. Hardware artifact: `build/vita-full/SmashMeleeVita-v3.47-ftdata-x5c-hsd-native.vpk`, 3,196,408 bytes, SHA-256 `41d81a7e6dd8437ffdebe1225814976acfa1d2400d782c9b197630278bb5bb33`; it is byte-for-byte identical to the freshly generated `SmashMeleeVita-assets.vpk`. `eboot.bin` is 2,765,045 bytes with SHA-256 `f46a6c8bd12ad80d087d6d25d7d95cb3a798782b03361eccf8cb023bcac9c32c`. The unstripped `melee_vita` ELF is 9,202,376 bytes with SHA-256 `0e3598236c041629dfded3b77395130d858151317ce4147fc4ea7a4b58083788`. The packaged SFO is 912 bytes, has valid `\0PSF` magic and reports APP_VER `00.57`.

Next physical test: Classic -> select character -> START. For Mario the expected new proof is `VITA_FIGHTER_PARTS_HSD_RAW_NATIVE_PASS file=? root=ftData.x5C off=0002a840 ...`; the old `mobj has unexpected blending flags (0x3c001000)` stop should disappear. The desired visible milestone remains the retail VS/loading presentation; physical visibility is not claimed until hardware validation.

## 2026-09-11 — v3.46: nativeize fighter costume-TObj u16 indices before the VS/loading model setup

The physical-v3.45 run advances farther through the restored retail `GS_INTRO_EASY` path: `GmIntEz.dat`, `IrAls`, `IrRdMap`, PlCo and Mario gameplay data all pass, and the Mario model graph reaches `VITA_FIGHTER_HSD_RAW_NATIVE_PASS ... PlyMario5K_Share_joint ... joints=61 dobjs=59 mobjs=59 pobjs=68 tobjs=60`. It then stops deliberately in `ftParts_80075240()` at the retained `can't find tobj!` assertion (`src/melee/ft/ftparts.c:793`). The supplied core again reports `stop_reason=0x30002` with IFAR/DFAR zero, so this is an intentional panic path rather than a Data Abort.

The core identifies the serialized mismatch exactly. The active Mario `Fighter` has `n_costume_tobjs=2`, but its runtime index array contains `0x0E00` and `0x0F00` (3584 and 3840). The original PAL `PlMr.dat` stores the same two entries as big-endian u16 values `00 0E` and `00 0F`, i.e. retail indices 14 and 15. `ftData_x8.x8.xC` is a relocation-managed table of pointers to these scalar u16 arrays: HSD relocation makes the pointers native, but the u16 payload itself remained GameCube-endian, causing `ftParts_80075240()` to search for TObj 3584 and exhaust the model's TObj list.

v3.46 extends the existing pre-relocation fighter nativeizer to the complete `ftData_x8.x8.xC` schema. It validates the costume-TObj count (`<=5`), validates the pointer/null table shape and bounded span, deduplicates shared index arrays, bounds-checks every referenced u16 entry, and converts each index to native endian before HSD relocation. A roster-wide PAL audit covers all 33 fighter archives and finds 28 unique reachable costume-TObj index arrays containing 58 total u16 indices; the permanent audit now validates that whole class and reports `GAMEPLAY_DAT_AUDIT_PASS fighters=33 motions=10658 demos=471 costume_tobj_arrays=28 costume_tobj_indices=58 plco=1 itco=2 articles=94/94 hurts=24/24 dynamics=3/3`. The original lookup assertion remains enabled; a Vita-only diagnostic `VITA_FIGHTER_TOBJ_LOOKUP_FAIL requested=%d available=%d dobjs=%u` now precedes it if a genuine later mismatch remains. Successful conversion emits `VITA_FIGHTER_TOBJ_INDEX_NATIVE_PASS`.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.46`; VPK APP_VER is `00.56`; `MELEE_VITA_SKIP_CLASSIC_INTRO=OFF` remains in the authoritative build cache. Managed and authoritative builds both complete through ARM compile/link, VELF, SELF and VPK with exit 0, and `git diff --check` plus the expanded PAL gameplay-DAT audit pass. Hardware artifact: `build/vita-full/SmashMeleeVita-v3.46-costume-tobj-u16-classic-intro.vpk`, 3,195,458 bytes, SHA-256 `60c85cbc4d321648308b8f7ad65dd12859402ef90a561eff26d7e352e36d559e`; it is byte-for-byte identical to the freshly generated `SmashMeleeVita-assets.vpk`. `eboot.bin` is 2,764,518 bytes with SHA-256 `a12299c135dd5b5d20f848db03c7c1daa665104d61886768840428bb6366b4cd`. The unstripped `melee_vita` ELF is 9,197,552 bytes with SHA-256 `460c31704fe47d7aeaa32ec9a733ad1b55406dc037a37740bdf7f097fd4d7a4d`. The packaged SFO is 912 bytes, has valid `\0PSF` magic and reports APP_VER `00.56`.

Next physical test: Classic -> select character -> START. For Mario the expected proof is `VITA_FIGHTER_TOBJ_INDEX_NATIVE_PASS ... root=ftDataMario count=2 arrays=1 indices=2`; the previous `can't find tobj!` stop should disappear. The desired visible milestone remains the actual retail VS/loading presentation; physical visibility is not claimed until hardware validation.

## 2026-09-11 — v3.45: correct the retail FtSFX x1C pointer that stopped the real GS_INTRO_EASY path

The physical-v3.44 run proves that the temporary Classic-intro bypass is gone and that the retail `GS_INTRO_EASY` path is now executing. `GmIntEz.dat` reaches `VITA_CLASSIC_EASY_TABLE_NATIVE_PASS`, the intro archives `IrAls` and `IrRdMap` are nativeized/loaded, and CObj setup begins. The run then stops deliberately at the gameplay-DAT fail-closed boundary while nativeizing `PlMr.dat`: `VITA_GAMEPLAY_DAT_INVALID kind=ftDataMario detail=attempted scalar swap on relocation off=000025b4`. The supplied core is again an assert/panic stop (`stop_reason=0x30002`), not a Data Abort.

The original PAL `PlMr.dat` identifies the mismatch exactly. `ftDataMario.x4C_sfx` points to `0x2598`, making the rejected relocation `0x25b4` equal to `FtSFX + 0x1C`. The retail relocation table marks three optional `FtSFXArr*` fields at `+0x00`, `+0x1C` and `+0x20`; for Mario they point to arrays with counts 5, 1 and 2 respectively. The decomp declaration had `FtSFX.x1C` as `int`, even though runtime code stores both `x1C` and `x20` in `fp->dmg.x190C` and later passes that value to `ft_800889F4(..., FtSFXArr*)`. A roster-wide PAL audit confirms that `+0x1C` is an optional relocation in most fighter archives, not a Mario-specific exception.

v3.45 changes `FtSFX.x1C` to `FtSFXArr*`, excludes `+0x1C` from scalar endian conversion, validates/nativeizes all three optional SFX arrays, and extends `vita/tools/audit_gameplay_dat.js` so the two scalar ranges are explicitly relocation-free while offsets `0x00/0x1C/0x20` are validated as pointer/null fields. The complete PAL audit remains green: `GAMEPLAY_DAT_AUDIT_PASS fighters=33 motions=10658 demos=471 plco=1 itco=2 articles=94/94 hurts=24/24 dynamics=3/3`. No fail-closed guard or retail assertion was removed.

Runtime marker is `MELEE_VITA_GAME_BOOT v3.45`; VPK APP_VER is `00.55`; `MELEE_VITA_SKIP_CLASSIC_INTRO=OFF` remains in the authoritative build cache. Both managed and authoritative source builds complete through ARM compile/link, VELF, SELF and VPK. Hardware artifact: `build/vita-full/SmashMeleeVita-v3.45-ftsfx-relocation-classic-intro.vpk`, 3,194,895 bytes, SHA-256 `ad336d52c0fed4097e62605ae5d7e180f23640161c4a8d3393d6d1f3359cb840`; it is byte-for-byte identical to the freshly generated `SmashMeleeVita-assets.vpk`. `eboot.bin` is 2,763,779 bytes with SHA-256 `5681162f3bdf61fdbe5f2d7eb6fc40a04674e52fc59cc24aeb4b1c07f3dfe2d2`. The unstripped `melee_vita` ELF is 9,196,992 bytes with SHA-256 `f17ddf207e2b97efbeb0ab019e87ed8e5d1b2abf6ea8367c1e6191f9482868a2`. The packaged SFO is 912 bytes, has valid `\0PSF` magic and reports APP_VER `00.55`.

Next physical test: Classic -> select character -> START. v3.44 already proves the real intro scene is entered and its assets are loaded; v3.45 removes the fighter-data guard that stopped it before presentation. The desired next visible milestone remains the retail VS/loading screen itself. Physical visibility is not claimed until the Vita test.
