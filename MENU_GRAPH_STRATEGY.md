...
## 2026-09-13 — v3.67: original major-mode continuation, deterministic SIS cameras and native Stage Select ShapeSet

The v3.66 physical log changes the menu-graph diagnosis. The user reaches the original main menu, 1P menu and Training CSS; the runtime captures ten SIS commands in the main menu and two SIS objects in CSS. Therefore the missing menu labels, CSS difficulty text and character name are not explained by absent strings, a missing font archive or a synthetic menu implementation. They are generated by original SIS but can be transformed with the wrong camera. The route is also definitely Training: `GameModeKind` 28 is `GM_TRAINING`; the previous `ADVENTURE(28)` text was a logging error.

v3.67 stores the exact HSD camera on each Vita SIS context/text and uses it during capture. GX-link masks remain only a fallback because multiple live menu and overlay cameras can share a mask. The gobj-less SIS path now owns a 4x4 orthographic matrix and declares it as `GX_ORTHOGRAPHIC`, fixing the previous 4x4 write into a 3x4 stack object and the inconsistent perspective type. The original 146,944-byte atlas remains linked byte-for-byte from `main.dol`. These are systemic fixes for all three reported text sites; visibility and placement still require the next physical run. Per-frame `capture_sig` values now cover geometry, matrices, materials and texture references; `changed_initial`/`changed_sample` in the main-menu markers will state whether original HSD animation data is actually changing.

The Stage Select edge failed before it could become visible. Set 10 of `MnSlMap.usd` has a valid `POBJ_SHAPEANIM` descriptor (`flags=0x9000`) which the compact HSD builder rejected, causing the explicit panic at `mnstagesel.c:461`. v3.67 converts the complete `HSD_ShapeSetDesc`, vertex/normal descriptors and pointer arrays to ARM32 and decodes big-endian shape samples in the original HSD CPU blender. The exact failing set now passes nativeization, archive relocation, `HSD_JObjLoadJoint()` and release in the linked ARM regression. No assertion was relaxed and vitaGL remains the renderer.

The main graph correction is the removal of the post-SSS terminal path. Previously the Vita route manually entered the match state and returned without ever running `gm_801A4D34`; consequently a loading screen or playable match was impossible by construction. Now CSS and SSS retain their native Vita capture bridge, but their state callbacks feed back into the complete original `GameMode` state machine. Confirmation advances through retail preload and the original Training/VS scene loop; SSS Back follows the original `gm_SetNextGameModeStateId(0)` edge to CSS. Other terminal modes use the full `gm_GetAllGameModes()` table instead of an unsupported return. All major-mode `on_init` callbacks execute once at global startup, matching the retail `gm_801A4510` lifecycle.

Current graph status:

- proven on v3.66 hardware: opening/title/main menu -> 1P -> Training CSS -> CSS exit -> SSS entry;
- fixed and linked in v3.67: SSS set-10 ShapeSet, SSS render construction, CSS/SSS forward and Back state ownership, original mode continuation and retail scene frame loop;
- source/offline proven: all 45 mode IDs remain accepted by the menu boundary, all mode initializers run in the ARM bootstrap, real MnSlMap set 10 loads through original HSD, SIS atlas matches retail data, full VPK packages successfully;
- pending physical proof: corrected SIS text placement, animated menu behavior, visible SSS, loading transition, first `GS_TRAINING` present and input inside a match.

The build keeps `MELEE_VITA_FULL_GAMEPLAY_SCENE=ON`, `MELEE_VITA_SKIP_CLASSIC_INTRO=OFF`, runtime v3.67 and APP_VER 00.77. Test artifact: `build/vita-full/SmashMeleeVita-v3.67-original-gamemode-sss-shapeset-sis.vpk`, 3,283,543 bytes, SHA-256 `a09464f815e5cc40655be63b91327cab38ada4e1384f097d97a8276a9ee9b59f`. The next graph traversal is Main -> 1P -> Training -> CSS -> SSS -> `GS_TRAINING`, followed by SSS Back -> CSS in a second run. Required new markers are `GAME_MODE_TABLE_INIT_PASS`, `GAME_MENU_LIVE_PASS ... capture_sig=... changed_initial=...`, `GAME_SSS_NATIVE_PREPARE_PASS ... shapeanim=native`, `GAME_SSS_CAPTURE_INIT_PASS`, `GAME_MODE_RETAIL_CONTINUE_BEGIN`, `VITA_RETAIL_STATE_BEGIN`, `VITA_RETAIL_SCENE_ONENTER_PASS` and `VITA_RETAIL_PRESENT`. The first absent marker defines the next integration boundary.
