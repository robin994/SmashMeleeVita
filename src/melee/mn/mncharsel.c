...
#else
    /* No GameCube camera callback on Vita, but SIS capture needs the original
     * camera-to-GX-link relationship to resolve text projection. */
    gobj->gxlink_prios = 0x1F;
#endif
...
void mnCharSel_Scene_OnEnter(void* arg0)
{
    ...
    mnCharSel_804D6CD4 = NULL;
    {
        ...
        HSD_ASSERTREPORT(0x14D0, mv_css_vita_prepare(&native_table) == 0,
                         "Vita CSS native asset conversion failed\n");
        css_data_table = (MnSelectChrDataTable*) native_table;
        css_models = &css_data_table->models;
    }
    mv_css_vita_trace("GAME_CSS_NATIVE_TABLE_READY");
#else
    if (lbLang_IsSavedLanguageJP() != 0) {
...
#endif
#ifdef MELEE_VITA_PLATFORM
    mv_css_vita_trace("GAME_CSS_SIS_BEGIN");
#endif
    if (lbLang_IsSavedLanguageJP() != 0) {
...
#ifdef MELEE_VITA_PLATFORM
    mv_css_vita_trace("GAME_CSS_SIS_PASS");
    mv_css_vita_trace("GAME_CSS_BUILD_BEGIN");
#endif
    mnCharSel_804D6CF4 = 0;
    mnCharSel_802640A0();
#ifdef MELEE_VITA_PLATFORM
    mv_css_vita_trace("GAME_CSS_BUILD_PASS");
#endif
}
