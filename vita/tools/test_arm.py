...
if args.assets:
    ...
    native_graph_address = arm.alloc(60)
    ...
    runtime_stats_address = arm.alloc(5 * 4)
    assert arm.call('mv_hsd_runtime_probe', native_graph_address, runtime_stats_address) == 0
    runtime_stats = struct.unpack('<5I', arm.uc.mem_read(runtime_stats_address, 20))
    assert runtime_stats == (
        native_reference['joints'], native_reference['dobjs'], native_reference['mobjs'],
        native_reference['pobjs'], native_reference['tobjs'])
    print(f"ARM upstream HSD_JObjLoadJoint: PASS ({runtime_stats[0]} joints, "
          f"{runtime_stats[1]} DObjs, {runtime_stats[4]} TObjs)", flush=True)
    capture_stats_address = arm.alloc(12 * 4)
    assert arm.call('mv_hsd_gx_capture_probe', native_graph_address, capture_stats_address) == 0
    ...
    tev_words = struct.unpack('<' + 'I' * (tev_record_count * 9),
                              arm.uc.mem_read(tev_records_address, tev_record_count * 9 * 4))
    for i in range(tev_record_count):
        record = tev_words[i * 9:(i + 1) * 9]
        print('ARM custom TEV command=%u payload=%s active=%08x' %
              (record[0], ' '.join(f'{word:08x}' for word in record[1:8]), record[8]),
              flush=True)
    ...
    multitex_words = struct.unpack('<' + 'I' * (multitex_record_count * 28),
                                   arm.uc.mem_read(multitex_records_address,
                                                   multitex_record_count * 28 * 4))
    for i in range(multitex_record_count):
        r = multitex_words[i * 28:(i + 1) * 28]
        f0 = struct.unpack('<f', struct.pack('<I', r[7]))[0]
        f1 = struct.unpack('<f', struct.pack('<I', r[19]))[0]
        uv0 = [struct.unpack('<f', struct.pack('<I', w))[0] for w in r[8:14]]
        uv1 = [struct.unpack('<f', struct.pack('<I', w))[0] for w in r[20:26]]
        print('ARM multitex command=%u count=%u rendermode=%08x ' % (r[0], r[1], r[2]) +
              'L0 flags=%08x image=%08x size=%ux%u fmtwrap=%08x blend=%.6g uv=%s active=%08x ' %
              (r[3], r[4], r[5] >> 16, r[5] & 0xffff, r[6], f0,
               ','.join(f'{x:.6g}' for x in uv0), r[14]) +
              'L1 flags=%08x image=%08x size=%ux%u fmtwrap=%08x blend=%.6g uv=%s active=%08x valid=%08x' %
              (r[15], r[16], r[17] >> 16, r[17] & 0xffff, r[18], f1,
               ','.join(f'{x:.6g}' for x in uv1), r[26], r[27]), flush=True)
