...
static int joint_channel_type_supported(uint8_t type)
{
    /* JObjUpdateFunc accepts the regular SRT/path/visibility channels plus
       user callbacks and the particle/sound/target callbacks.  The 0x32+
       matrix channels are generated by RObj constraints as Vec3 updates, not
       serialized scalar FObj streams, so keep those out of this decoder. */
...
static int apply_joint_channel(void *opaque, uint8_t type, float value)
{
    ...
    if (!joint_channel_type_supported(type)) return 1;
    if (!isfinite(value)) return -1;
    /* The compact sample surface only stores the twelve ordinary JObj
       channels.  Callback channels (20..42) are still parsed/validated but
       are intentionally not folded into the 12-bit channel vector. */
    if (type <= 12) {
...
static int sample_aobj_desc(const MvDat *dat, uint32_t aobj, float frame,
                            ...
                            uint32_t *flags_out, float *end_frame_out,
                            uint32_t *obj_id_out, int allow_obj_ref)
{
    ...
    uint8_t has_obj_id = 0;
    if (read_pointer(dat, aobj + 12, &obj_id, &has_obj_id)) return -1;
    if (flags_out) *flags_out = aobj_flags;
    if (end_frame_out) *end_frame_out = end_frame;
    if (obj_id_out) *obj_id_out = has_obj_id ? obj_id : 0;
    if (has_obj_id && !allow_obj_ref)
        return 1; /* Runtime native proxy still needs a matching JObj ID graph. */
                         ...
                         MvAnimJointSample *sample)
{
    ...
    return sample_aobj_desc(dat, aobj, frame, sample, apply_joint_channel,
                            &sample->fobj_count, NULL, NULL, NULL, 0);
}
...
int mv_aobj_validate_jobj(const MvDat *dat, uint32_t aobj_desc)
{
    ...
    memset(&sample, 0, sizeof(sample));
    size_t fobj_count = 0;
    return sample_aobj_desc(dat, aobj_desc, 0.0f, &sample,
                            apply_joint_channel, &fobj_count,
                            NULL, NULL, NULL, 1);
}
